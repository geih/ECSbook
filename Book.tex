...
%%%%% HALFTITLEPAGE
\begin{halftitlepage}
\halftitle{Biophysical modeling of electrical and magnetic brain signals}
\halfsubtitle{- how to compute LFP, ECoG, EEG and MEG signals}
...
\item Refer to main Sections as "Chapters", subsections as Sections? Answer: YES. This is also implicit in the Cambridge-template. 
	...
	\ghtxt{La inn ny kommando for appendix: {\textbackslash}label\{app:Aniso\}: \fref{app:Aniso}}. 
...
$V, V_\mathrm{i}, V_\mathrm{e}, V_\mathrm{m}$  	& \si{\volt} 	& electric potential (general, intracellular, extracellular, transmembrane) \\
$E,\mathbf{E}$		& \si{V/m} 	& electric field (scalar, directed) \\
$I, \mathbf{I}$ 				& \si{\ampere} 			& electric current (scalar, directed) \\
$i, \mathbf{i}$ 				& \si{A/m^2} 			& specific electric current (scalar, directed) \\
$\sigma$ 				& \si{S/m}  		& conductivity (for volume currents) \\
$C$ 				& \si{A/m^3}  		& current source density\\
$c_k$ 				& \si{\mol/m^3}  		&  ion concentration of species $k$ \\
$f_k$ 				& \si{\mol/(m^3s)}  		& source density of ion species $k$\\
$J_k, \mathbf{J}_k$ 				& \si{\mol/s} 			& flux of ione species $k$ (scalar, directed) \\
$j_k, \mathbf{j}_k$ 				& \si{\mol/(m^2s)} 	& flux density of $k$ (scalar, directed) \\
$g_k$ 				& \si{S/m^2}  		& specific membrane conductance for ion species $k$\\
$E_k$		& \si{\volt} 	& reversal potential for ion species $k$ \\
$c_\mathrm{m}$ 				& \si{F/m^2}  		& specific membrane capacitance \\
$\tau, \tau_\mathrm{m}$ 				& \si{s}  		& time constant (general, membrane) \\
...
\chapter*{Mathematical notation}6
