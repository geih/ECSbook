...
The extracellular potential can now be computed using the standard formalism based on the pattern dendritic membrane currents and the soma membrane current~\cite**{Pettersen2008a}:
%
\begin{equation}
  V_\mathrm{e}(z,\rho) = 
...
\end{equation}
%
\gex{Here $\rho$ is the radial distance from the recording point to the dendritic stick axis, and $z_\mathrm{soma}$ 
is the depth position of the soma, see  \Fref{fig:Spikes:ball-and-stick-model-sketch}. 
For the present analysis we will use this second option.}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.6\textwidth]{Figures/Spikes/Spikes-ball-and-stick-sketch}
\end{center}
