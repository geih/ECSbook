...
The high-pass filtering of the extracellular signal typically used \ehtxt{when detecting \sout{in extraction of}} spikes \ehtxt{in \sout{from}} extracellular recordings, 
changes the shape and reduces the signal power of the extracellular spike signal.  
\ehnote{rar setning:} The rationale for this high-pass filtering is 
not to modify the spike shape \ehtxt{but \sout{and}} make it more amenable for detection \ehtxt{and \sout{or}} spike sorting. 
It is \ehtxt{required \sout{rather}} to remove the low-frequency (LFP) part which stems from the other slower neural processes.  
This LFP signal typically dominates in terms of overall power, but has very little power above a few hundred hertz~\cite**{Pettersen2008,Einevoll2013}. \ghnote{Er ikke dette en tautologi? The low-frequency part has little power in the high frequencies?}
