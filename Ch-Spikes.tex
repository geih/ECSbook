...
Also, the shape of the spike is very different from the shape of intracellularly recorded action potentials (Fig.~\ref{fig:Spikes:Henze}).
...
This is illustrated in Figure~\ref{fig:Spikes:freq_dep} showing that also very narrow spikes may contain low-frequency components.
...
stems from other neural processes.  This LFP signal typically dominates in terms of overall power, but has very little power above a few hundred hertz~\cite**{Pettersen2008,Einevoll2013}.
...
strongly depending on the position at which it is recorded.  This is illustrated in Fig.~\ref{fig:Spikes:MultiCompartment} showing computed spikes at various 
positions around a neuron during the firing of an action potential. 
Here the scheme described in \gex{XX} is used with a biophysically detailed neuron model for a layer-5 pyramidal cell.
...
%with a comprehensive branching structure typical for real neurons as in Figure~\ref{fig:Spikes:MultiCompartment}.
...
%around the soma for the example pyramidal neuron in Figure~\ref{fig:Spikes:MultiCompartment}.  
...
%
%As for the two-compartment spike model, the spike amplitude in Figure~\ref{fig:Spikes:MultiCompartment} 
...
in Fig.~\ref{fig:Spikes:MultiCompartment} be explained by simpler neuron models? 
...
An example of a spike generated by such a model is shown in Figure~\ref{fig:Spikes:DifferentNeuronModels}D. Around the soma, characteristic
spikes with a sharp negative peak followed by a slower positive hump is seen, in accordance with typical experimental
spike recordings as exemplified in Figure~\ref{fig:Spikes:Henze}. The same spikes, though inverted, are observed outside
...
Unlike the two-compartment neuron model, the so-called ball-and-stick neuron model also exhibits position dependence of spike shape 
...
In particular, the width of the negative sodium peak is seen to increases with increased lateral distance.
...
Figure~\ref{fig:Spikes:ball-and-stick-results} shows the distance dependence of the spike amplitude and spike width both for a detailed multi-compartmental pyramidal neuron model and ball-and-stick neurons.
...
see Figure~\ref{fig:Spikes:ball-and-stick-neuron-models}.
The intracellular action potential shown in the inset in the left panel of 
Figure~\ref{fig:Spikes:ball-and-stick-frequency} was imposed as a voltage-clamp in the soma.
...
%\end{center}
%\caption[]{\textbf{Neuron models considered in results in Figure~\ref{fig:Spikes:ball-and-stick-results}}. 
...
Figure~\ref{fig:Spikes:ball-and-stick-frequency} illustrates the Fourier decomposition of the
...
\end{center}
\caption[]{(a) Action potential used in simulations in Figure~\ref{fig:Spikes:ball-and-stick-results}(inset) 
...
\end{equation}
%
and the spike is thus found to be
%
\begin{equation}
...
frequency (see Figure~\ref{fig:Spikes:ball-and-stick-frequency}). Further, $d$ is the diameter of the dendritic
...
closer to the soma for higher frequencies, as  seen in the inset in Figure~\ref{fig:Spikes:ball-and-stick-sketch}.  
...
As shown in Figure~\ref{fig:Spikes:ball-and-stick-sketch} this current dipole length will depend on frequency.
...
$\hat{i}_\mathrm{m}$ weighted with distance $z$ from soma, see Figure~\ref{fig:Spikes:ball-and-stick-sketch}. 
...
%dipole as illustrated in Figure~\ref{fig:Spikes:TwoCompartment}. A qualitatively similar dipolar pattern, although not so distinct,
%is also seen for the spike generated by the biophysically detailed multi-compartment neuron in Figure~\ref{fig:Spikes:DifferentNeuronModels}.
...
%the spike-amplitude panel in Figure~\ref{fig:Spikes:ball-and-stick-results}.
...
dipole as illustrated in Figure~\ref{fig:Spikes:DifferentNeuronModels}D. A qualitatively similar dipolar pattern, although not so distinct,
is also seen for the spike generated by the biophysically detailed multi-compartment neuron in 
Figure~\ref{fig:Spikes:DifferentNeuronModels}.
...
Figure~\ref{fig:Spikes:ball-and-stick-results}.
\gen{Nevn at noen bruker den deriverte av intracellular som "proxy". Sammenlign med Figur 1.1}.
...
%Thus close to the soma the spike is observed to be sharper than the intracellular action potential
%as observed in the insets in Figure~\ref{fig:Spikes:ball-and-stick-frequency}. 
%In the far-field regime there is no such high-frequency amplification ($\hat{V}_\mathrm{e,near}(f,r) \propto f^0 \sim 1$).
%As a consequence, spikes measured far away from the soma will have less high-frequency content than those measured close to soma.
%Thus the far-away spikes will be blunter and have larger spike widths as seen in the spike-width panel of 
%Figure~\ref{fig:Spikes:ball-and-stick-results}.
...
%Figure~\ref{fig:Spikes:ball-and-stick-results}, for example, the spike amplitude decays from being about
...
\gen{Add text on spikes for axonal bifurcations from Kempters group in Berlin}
...
is provided by Fig.~\ref{fig:Spikes:electrode_size}) showing spikes recorded with circular electrodes of different radii.
...
Figure~\ref{fig:Spikes:MEA-setup}.
...
An example result is shown in Figure~\ref{fig:Spikes:MEA-spikes}.
...
identify individual spikes from the recorded signal. If so, an estimate of the combined firing rate of the neurons surrounding the contact may be found in a different 
way,  that is, by rectification of the high-pass filtered extracellular potential~\cite**{Schroeder1998,Schroeder2001,Ulbert2001}. The principle behind this approach is illustrated
...
This approach was used to estimate firing rates of cortical populations of neuron in the rat barrel (somatosensory) cortex based on multielectrode laminar 
recordings~\cite**{Einevoll2007,Blomquist2009}. To test the validity of the approach, a modeling study was pursued where an analogous virtual (in silico) experiment 
...
mimicking a neuronal population in rat barrel cortex, was considered (\Fref{fig:Spikes:MUA-population}). The neurons received synaptic inputs simulating a volley of activation following 
sensory stimulation from flicking the appropriate whiskers. Extracellular potentials were computed for a set of contact positions along the central axis of the cylinder (panel B).
...
Here it can be seen that the frequencies up to 100 Hz dominates the signal completely, that is, has the highest amplitudes (and power, corresponding to the amplitude squared). 
...
While the spikes are buried by the low-frequency in components in the raw signal, they can be extracted by high-pass filtering as seen in panels C and D in
\Fref{fig:Spikes:MUA-ApicalSynapses}. \gen{Panel labeling is wrong in present figures}
The resulting signal after high-pass filtering and subsequent rectification for the example, as a function of time in panel C
and in frequency space in panel D. A first observation of this MUA signal in panel C is
that the remaining signal is confined to channels deeper in the cortex, that is, close to the somas of the neurons in the populations.
This reflects that extracellular signatures of action potentials are typically confined to within 100~$\mu$m from the soma, cf. \Fref{fig:Spikes:DifferentNeuronModels}. A second observation in panel C is that the MUA signal 
...
consists of a collections of sharp peaks of different heights. This `noisyness' stems from the inherent randomness both the time of spiking 
and somas position of the low number (40) of spiking neurons in the population. 

In sensory systems it has been common to measure 
trial-averaged responses, that is, the response averaged over many repetitions of the same stimulus. The rationale is that this procedure highlights the contributions from sensory input in that the effects of other inputs and noise to sensory neurons are reduced. The trial-averaged MUA found from averaging MUA results from forty model trials is shown in 
panel E of \Fref{fig:Spikes:MUA-population}, revealing a much less `noisy' signal. 

The obvious next question is to what extent the resulting MUA signal gives a good measure of the population firing rate. Unlike in experiments, the true underlying firing rate is known in the model, allowing for a quantitative comparison. The question was studied in detail for the model in question in \citeasnoun**{Pettersen2008}, and a result for the present example 
model is given in panel F in \Fref{fig:Spikes:MUA-ApicalSynapses}. Here a firing-rate estimate based on the MUA signal from one of the recording channels positioned at a depth in the midst of
somas of the population is compared with the true, temporally smoothed firing rate.  
As observed the overall agreement is quite good,

 One deviation is that the MUA signal predicts a non-zero firing rate prior to the onset of true firing, the reason being that the synaptic input current itself gives a contribution to the MUA signal. Another deviation is that the MUA-based firing rate predict a too low maximum firing rate, due to cancellation of signal from temporally overlapping spikes from different neurons. This latter effect increases with increasing firing rates and can be remedied by assuming a supralinear relationship between the MUA signal and firing rate \cite**{Pettersen2008}.  The accuracy of the population firing rate can also be improved by using MUA signals from several adjacent recording channels in the estimation \cite**{Pettersen2008}.  
 
The accuracy of the this MUA-based estimation of population firing rates will depend on the specific situation considered. In \citeasnoun**{Pettersen2008}, it was, for example, found
...
%spike shape around the neuronal soma (see Figure~\ref{fig:Spikes:MultiCompartment}) 
