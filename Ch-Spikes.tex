...
\gex{When modeling spikes with the ball-and-stick neuron, one option is to include spike-generating active sodium and potassium conductances in the soma compartment. Another option is to impose a waveform mimicking a somatic action potential as a boundary condition in the soma end of the dendritic stick. In both cases the spike shape and size will be determined by the pattern of membrane currents returning to the ECS through the dendritic stick, see \Fref{fig:Spikes:ball-and-stick-sketch}. For the present analysis we will use the second option. \ghnote{Blir det helt det samme, da? Hvis vi har med soma m/ AP-kanaler, inngaar vel somastroemmene i beregningen. Det er vel bare i tilfellet der vi trykker inn en waveform i dendritten at spiken er bestemt kun av stick-stroemmene slik du beskriver over?}
