...
Lower right: Comparison between estimated firing rates based on trial-averaged (40 trials) MUA-traces and the ?true? population firing rate (solid lines). 
The amplitude of the estimated firing rates have been fitted to minimize the relative square error.
Estimates ?r(t) for apically excited population based on average MUA signals from electrode contacts 13--17 (dashed line) and electrode contacts 14--16 (dotted line), cf 
\cite**{Pettersen2008}.
}
...
trial-averaged responses, that is, the response averaged over many repetitions of the same stimulus. The rationale is that this procedure highlights the contributions from sensory input in that the effects of other inputs and noise to sensory neurons are reduced. The trial-averaged MUA found from averaging MUA results from forty trials is shown in panel E of \Fref{fig:Spikes:MUA-population}.
