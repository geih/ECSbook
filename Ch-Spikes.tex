...
\subsection{Dependence of spike size and shape on neuronal properties}
...
increases \citep{Pettersen2008,Pettersen2012}. 

\citet{Pettersen2008} took advantage of the mathematical tractability of the ball-and-stick model to
...
\citet{Pettersen2008} derived simple formulae relating the intracellular action potential to the extracellular spike for two limiting cases:
...
\subsubsection{Spike amplitude dependence on distance}
...
%neurons \citep[Fig.~X]{Pettersen2008}.
...
\subsubsection{Spike amplitude dependence on neuronal parameters}
...
cf. \citet[Equation 23]{Pettersen2008}.)
...
\subsubsection{Spike shape dependence on distance}
...
\subsubsection{Generalisation of findings to other neuron models}
While the formulae above were derived for a neuron model with a single passive dendritic stick, similar expressions can be derived for 
more complicated neuron models where several passive sticks protrude from the soma, see \citet{Pettersen2008}.
The main conclusions above hold also for these neuron models, in particular that spike widths always increase with distance and that
the amplitude of a spike is proportional to $d^{k}$ where $k\sim1.5-2$. For neurons with many dendrites attached to the 
...
\subsection{\red{Spikes in neurons with active dendrites}} 
\begin{itemize}
...
\subsection{\red{Axonal contributions to spikes}}
\begin{itemize}
...
\subsection{\red{Multi-unit activity (MUA)} }
Kjernereferanse: \citep{Pettersen2008}


\subsection{\red{Insights from MUA studies}} 
