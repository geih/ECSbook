...
An approximate way of including the contribution to the spike from the dendritic return currents is to assume all return currents to leave the dendrite at a single point \ghtxt{\sout{on the dendrite}}. Then we are left with a current dipole where the transmembrane current entering at the soma is at all times balanced by an oppositely directed current with the same magnitude leaving at a single point on the dendrite. The current dipole length is then given by the distance between the soma and the dendritic position of the return current.  \ghnote{Er vi ikke tilbake til 2-comp-modellen naa? }

As shown in \Fref{fig:Spikes:ball-and-stick-sketch} this current \ghnote{Her da? Skal det ikke vaere bindestrek her?!} dipole length will depend on frequency. A natural choice is to choose the dipole length to be equal to the frequency-dependent length constant $\lambda_\mathrm{AC}(f)$ of the stick,
