...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{Lecture-InputResistance}
\caption{
%\textbf{}
}
...
\begin{itemize}
\item Finite cable \\
\Figure{STERRATT-FIG-2.18}
\end{itemize}

\subsection{Temporal solutions of cable equation \BookRef{- 2.9 in Sterratt}}

\begin{itemize}
\item General solution:
%%%%
\begin{equation*}
V(x,t) = C_0(x) e^{-t/\tau_0} + C_1(x) e^{-t/\tau_1} + \ldots
\end{equation*}
%%%%
$\tau_0$ is the \emph{membrane time constant}: $\tau_0=\tau_m=R_m C_m$ \\
($\tau_0 > \tau_1 > \tau_2 > \ldots$) \\
\Figure{STERRATT-FIG-2.17(b)}
%
\item For example in figure, the membrane potentials at all positions recorded is the same after about 2 ms
\item But note that the system has not yet reached steady state (back to resting potential)   
\item Explanation: Membrane time constant is 10~ms ($R_m=1\;\Omega\text{m}^2,\;C_m=1\;\mu\text{F}/\text{cm}^2=0.01\;\text{F}/\text{m}^2$ $\Rightarrow$ $\tau_m=R_m C_m$=10~ms)
\end{itemize}
\Comment{BOX 2.7: Eccles vs. Rall}

%%%%%%%%%%%%%
%%%CH4
%%%%%%%%%%%%%
%\include{FYS388-LecNotes-Ch4-Synapses}
