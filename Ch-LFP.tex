...
Jeg tror dette er paa sin plass, fordi: Vi alltid uttrykker en viss usikkerhet angaaende opphavet til LFP, det er hvertfall litt kontroversielt, og vi paapeker at LFP er vanskelig aa tolke.
}
...
Laminar distribution makes sense.

The time-course of LFP signals are perfectly in line with expectations from synapses, and synaptic input times.

We lack any other reasonable candidate: Spikes can certainly be expected to contribute somewhat, but can 

Mono-synaptic unitary LFPs: First spike, then LFP \cite**{Swadlow2002}, is perfectly in sync with modelling \cite**{Hagen2017}.


\section{\red{Single-neuron LFPs}}
...
\section{\red{Intrinsic dendritic filtering}}
...
LFPs are often analyzed in terms of the frequency content. Many things affect how much power the LFP has at different frequencies, and different parameters can often affect different frequency regions. The dynamics of the synaptic input mostly affect low-frequency (\fref{fig:LFP:psd_input}).
\begin{figure}[!ht]
\begin{center}
...
\tvnnote{placeholder. Vet ikke om vi skal ha dette.}
}
