...
As for the two-compartment spike model, the spike amplitude in Figure~\ref{mm:fig:EP-spike-MultiCompartment} 
...
Left panel shows the EP at different spatial positions, while right panel shows the corresponding
soma membrane potential during the action potential. 
\todo{Figure + caption to be updated. Will change to Hay-model (which Bruce also will use).}
}
...
\begin{figure}[t]
  ...
  \caption{\emph{Simulation of \term{extracellular field potential}s. Extracellular
    ...
    reversed.} 
        \todo{Revise: Figure and caption copied directly from first edition. Must be adapted.}}
  \figpermOurs
...
\todo{DCS: The frequency dependence here comes from intracellular properties? If so, would be great to make it clear.}
...
\todo{Figure + caption to be updated.
...
\todo{DCS: Intuition for the relationship witd $d, C_m, f, r_a$.}
...
\todo{Figure + caption to be updated. Adapted from \citet{Pettersen2008}.}
}
...
\todo{Figure + caption to be updated. Adapted from \citet{Pettersen2012}.}
...
%
\centerline{\includegraphics{Figures/mm/EP-spike-sorting-w35-r300}}\vspace*{6pt}
%
A sharp electrode placed in brain tissue will pick up spiking signals from several neurons. 
However, the shapes of the spikes will be different for the different neurons, and this can be used to sort the spikes according to their neurons of origin. This is referred to as \firstterm{spike sorting}, 
\todo{'spike sorting' is in 'firstterm' but not shown as bold nervertheless.}
\todo{DW suggest a hyphen, that is, 'spike-sorting', but this seems incorrect to me.}
...
\todo{GTE: Figure to be adapted from Quiroga (2007).}
\end{boxfloat}
%%%
...
where electrophysiological cellular properties can be studied for a few hours. Such \firstterm{\emph{in vitro}} recordings
...
the brain tissue is placed on a \firstterm{micro-electrode array (MEA)}. Here the bottom of the device contains a grid of electrode
...
%
\todo{DW: Put text around figure to save space?}
...
\firstterm{method of images} from electrostatics, see Box~\ref{braintissue:box:MOI}.
...
%
\centerline{\includegraphics{Figures/mm/MEA-2-w43-r300}}\vspace*{6pt}
%
\todo{DW: Too small.}
\end{boxfloat}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
