...
\end{center}
\caption{\textbf{Multicompartmental modelling.}  The neural morphology (A) is represented by a multicompartmental model, where the compartments are treated as cylinders connected by resistors (B). Electric currents in the model can be separated into two groups (C): (i) transmembrane current in a compartment (green arrows), and (ii) intracellular currents between compartments (yellow arrows). 
}
...
It can be shown that the temporal solution for $V$ in a passive cable is \cite**{rall1969}:
\begin{equation}
...
Fig. Fig. \ref{Neuron:fig:Semiinf}B shows the the membrane potentials at selected positions along a $1000 \, \mu$m long cable responding to a stimulus resembling a synaptic input in one end. We see that the peak response comes faster in proximal than in distal location, and that the peak response decays with distance from the synapse. However, the potential is about the same at all points along the cable a few milliseconds after the stimulus was delivered. After this, $V$  decays gradually towards the resting potential. This decay takes place at the slower time scale of the membrane time constant, which in this simulation was 10~ms ($R_m=10\,\text{k}\Omega \text{cm}^2$, and $c_m=1\,\mu\text{F}/\text{cm}^2$). Although the dynamics of course will be system specific, depending on the membrane time constant and generally on the presence of active mechanisms, the simulation in Fig. \ref{Neuron:fig:Semiinf}B gives a general, qualitative idea on how signals spread in dendrites.
...
For example, when an K$^+$ channel opens, (i) diffusion will strive to drive K$^+$ out from the neuron, since the intracellular space is more K$^+$-rich than the extracellular space, while electric drift will strive to drive K$^+$ into the neuron, since the membrane potential $V$ (defined as intracellular minus the extracellular potential) tends to be negative ($V \sim - 70$  mV during rest). Normally, the diffusive process will dominate, so that the net flux of K$^+$ is outward. The outward flux of K$^+$ will cause a gradual decrease (hyperpolarization) of the membrane potential,  and a corresponding gradual increase of the drift component. If the K$^+$ channel stays open, the hyperpolarization will go on until the drift component becomes equal in magnitude to the diffusive component. 
