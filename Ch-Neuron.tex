...
Neuronal modeling as presented here, based on the combination of a Hodgkin-Huxley type formalism for membrane mechanisms, and cable theory for intracellular signal propagation, relies on a series of important assumptions. 

Firstly, Hodgkin-Huxley type ion channels do have some limitations when it comes to capturing the exact electrophysiological dynamics of ion channel gating, something that has inspired the development of detailed ion channel models based on Markov type kinetics, which typically are characterized by a large number of states and related transitions (see e.g., \cite{balbi2017}). Secondly, the cable equation is based on the assumption that the neuronal morphology can be considered as a one-dimensional structure, i.e., 

amount of details that one includes in the model of an ion channel, 

Cable equation 1D
\cite{lindsay2004maxwell}

actual ion channel gating are modeled with, 


Neural modeling, as presented here, is
