...
Conductivity is related to conductance $g$ through $\sigma = gd$, where $d$ is the condu

and will depend on what ion channels that are present and activated. The ratio \fref{eq:Sigma:ratiocondition} will be greatest when the conductance is at its minimal, and as an estimate of the minimal conductance, we use the leakage conductance in In the HH model (see \fref{box:Neuron:HH}): $\bar{g}_{\mathrm{L}} &=& 0.3 \,\si{\milli\siemens\per\square\centi\metre} = 30 \,\si{\siemens\per\square\metre}$. This corresponds to a conductivity $\sigma_\text{leak} = 30 \times 10^{-8} = 3\times10^{-7} \si{\siemens\per\metre}$, and gives a charge relaxation time $\tau = \epsilon/\sigma_\text{leak} \approx 0.3 \si{\milli\seconds}$ for the membrane medium. For $f = 1000 \, \si{\hertz}$, $2 \pi f \tau \approx  2$. Hence, the criterion in \fref{eq:Basics:ratiocondition2} is clearly not met. We note that a membrane time constant of $\tau \approx 0.3 \si{\milli\second}$ is very low. In the HH model,  the active sodium and potassium conductances are partially open even at the resting potential, so that the conductivity is never as low as $$\bar{g}_{\mathrm{L}}$, and the passive membrane time constant for this model is about $0.85\,\si{\milli\second}$ (see chapter 6.2.3 in \citeasnoun**{Koch1999}). Also, the HH model has a faster time constant than what is estimated for most mammalian central neurons, for which the typical values range from 20 to 100 \si{\milli\second} \cite**{koch1996}. With $\tau = 30 \,\si{\milli\second}$, and  $f = 1000 \, \si{\hertz}$, \fref{eq:Basics:ratiocondition2} gives the ratio $2 \pi f \tau \approx  0.02$



 and capacitive effects can not be neglected. We note that a membrane time constant of $\tau \approx 0.3 \si{\milli\second}$ is lower than 
...
We note that the HH-type formalism for ion channels is a simplified approximation of the electrophysiological dynamics of ion channel gating and ion channel currents. Since the HH-type formalism is based on deterministic equations, it cannot account for ion channel noise, which can stem from the stochastic gating at the single channel level \cite**{Meunier2002, Goldwyn2011}. Also, when channels open and close, movement of charges in channel proteins give rise to a tiny current called
the \textit{gating current}, which is not included in the HH formalism \cite**{Hille2001}. Furthermore, the relatively simple gating kinetics described by HH-type channels is not accurate for all ion channels, and more detailed ion channel models have been developed, based on a formalism more compatible with statistical physics and thermodynamics \cite**{Destexhe2010}, or even more detailed, Markov-type formalism based on single-channel recordings \cite**{sakmann2013}.
