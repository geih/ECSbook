...
(cf. eq. \ref{Neuron:eq:revpots}) and can have dramatic effects on neuronal firing properties 
...
\Fref{Eldiff:eq:EDCSD} is the electrodiffusive counterpart to eq. \fref{VC:eq:CSD2}, i.e., to the 
...
This definition allows us to split \Fref{Eldiff:eq:EDCSD} into two equations:
\begin{equation}
\nabla \cdot (\sigma_\text{t}\nablaV_\text{VC}) = - C, 
\label{Eldiff:eq:EDCSDVC}
\end{equation}
and
\begin{equation}
\nabla \cdot (\sigma_\text{t}\nablaV_\text{d}) = - F\alpha \nabla \cdot \left (\sum_k{z_k \tilde{D}_k{\bf \nabla} c_{k}} \right).
\label{Eldiff:eq:EDCSDdiff}
\end{equation}




For a more direct comparison, we can multiply eq.  \ref{Eldiff:eq:eldiffCSD2} with the extracellular volume fraction $\alpha$, and get:
