...
However, as we explained in \fref{sec:Neuron:Ions_and_reversals}, 
the number of ions that cross a neuronal membrane during the generation of
an action potential (or other brief neuronal event) is too small to have a notable impact
on the ion concentrations inside a neuron. 
Short-term concentration changes on the cellular level are therefore normally negligible.
...
the homeostatic machinery is therefore not explicitly modeled, but simply assumed to do its job, 
and accordingly, ion concentrations are assumed to be constant. 

While the assumption of constant ion concentrations probably holds fairly well during normal cellular activity, 
there are scenarios when concentrations can deviate quite dramatically from baseline, 
...
and standard MC models, accounting only for the latter, become inaccurate \cite**{Qian1989,Saetra2021}. 
...
\cite**{Gardner-Medwin1981,Dietzel1989,Herreras1993}. 
If they are present, these diffusion potentials will give contributions to the total extracellular potential ($V_\text{e}$)
that are not accounted for by standard volume conductor (VC) theory \cite**{Halnes2016,Halnes2017,Solbraa2018}.
