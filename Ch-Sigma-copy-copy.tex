...
occupies only about 20\% of the the total tissue volume, and has a highly tortuous \index{Tortuosity} geometry, 
...
so that brain tissue can be treated as a continuous, porous medium \cite**{Nicholson1981,Gratiy2017}. 
...
If capacitive effects are present in tissue currents, 
these can be accounted for is the CSD equation (\fref{eq:Basics:continuity2}) 
by allowing the tissue conductivity to be complex:
\begin{equation}
{\bf \nabla} \cdot \left(\sigma_\text{t}(\omega) {\bf \nabla} {\phi} \right) = -C,
\label{eq:Sigma:compexCSD}
\end{equation}
where $\sigma_\text{t}(\omega)$ depends on the the angular frequency $\omega = 2 \pi f$ of the electric field. 

To define this complex conductivity, we start with the total (conductive plus capacitive) 
current density through the medium:
\begin{equation}
{\bf i} = \sigma_\text{R}{\bf E} +  \epsilon \frac{\partial {\bf E}}{\partial t},
\label{eq:Sigma:itot}
\end{equation}
where $\sigma_\text{R}$ denotes the real (purely resistive) part of the tissue conductivity, 
i.e., so that in the case of a purely resistive medium, we get that $\sigma_\text{t} = \sigma_\text{R}$.
...
\label{eq:Sigma:Eomega}
\end{equation}.
where $j$ is the imaginary unit, and $\omega$ the angular oscillation frequency of field (or a component of it). 
If we insert \fref{eq:Sigma:Eomega} into \fref{eq:Sigma:itot}, we can write the current density as:
\begin{equation}
{\bf i} = \left( \sigma_\text{R} + j\omega \epsilon \right) {\bf E}.
\label{eq:Sigma:itot2}
\end{equation}
Further, we realize that we can write this on the linear (ohmic) form
\begin{equation}
{\bf i} = \sigma_\text{t} {\bf E}, 
\label{eq:Sigma:itot3}
\end{equation}
where we have introduced the complex conductivity:
\begin{equation}
