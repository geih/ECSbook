...
\end{eqnarray}
where $\langle V_\mathrm{m}(t) \rangle$ denotes the spatiotemporally averaged postsynaptic potential. 
We here use the sub-fix $Y|X$ notation to emphasize that these parameters may be connection specific.
...
\begin{equation}
H_{ji}(\mathbf{r},\tau) \coloneq \mathbf{F} \left[ \overline{i}_{\mathrm{syn}YX} \left( \Delta_{YX} \ast f_{YX} \ast G_{ji} \right)(\mathbf{r}, \tau) \right]~, 
\end{equation}
where $\Delta_{YX}(t) \coloneq \delta(t - d_{YX})$ and
the Green's like functions $G_{ji}(\mathbf{r}, \tau)$ encompass all dendritic filtering effects on the synaptic input currents into each corresponding distribution of transmembrane currents $\mathbf{I}_\mathrm{m}(\mathbf{r}, \tau)$.
...
computing the full set $H_{ji}(\mathbf{r},\tau)$ becomes intractable as the number of connections (and consequently number of kernels) grow proportionally to the product $N_X N_Y$ if the connection probabilities are kept fixed. 
