...
by construction $H_{YX}(\mathbf{r}, \tau) \coloneq 0$ for $\tau < 0$ as any contribution to the signal $\phi(\mathbf{r}, t)$ is solely due to postsynaptic contributions which in this scenario can not occur before the presynaptic spike event.
...
Can we deduce a methodology to compute the set of $\hat{H}_{YX}(\mathbf{r}, \tau)$ we need for all pre- and postsynaptic populationso $X$ and $Y$ directly,
...
We also assume that each postsynaptic population can be represented by \emph{one} typical biophysically detailed neuron model.
