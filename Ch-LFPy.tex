...
Here, $R_\mathrm{a}$  and $d$ is the axial resistivity (\si{\ohm\metre}) and diameter (\si{\metre}) of the dendritic cable model, respectively \ehnote{crossref chap. 3}.  
...
For a number $N$ child compartments connected at the end point location $(x_j+L_j/2)$ of a parent compartment (\Fref{fig:LFPy_circuits}C) the potential can be computed from the parent- and child-compartment mid-point potentials as \cite{Hagen2018}
...
%
\begin{eqnarray}
R_{j+n} &=& \frac{4 R_\mathrm{a}}{\pi d_{j+n}^2} \frac{L_{j+n}}{2} \nonumber \\
		&=& \frac{2 R_\mathrm{a}L_{j+n}}{\pi d_{j+n}^2} ~.
\end{eqnarray} 
\ehnote{Check!}
...
Consistent with the point-source formalism \ehnote{crossref eq in sec 3}, 
we let the `location' of each axial current be the mid-point $\mathbf{r}_j$ of each cylindrical compartment $j$. 
In a similar manner we will represent the total transmembrane currents per compartment as a matrix
\begin{equation}
\mathbf{I}_\mathrm{m}(t) = 
\begin{pmatrix}
i_\mathrm{m}(x_1, t) || \mathbf{d}_1 || \\
i_\mathrm{m}(x_2, t) || \mathbf{d}_2 || \\
\vdots \\
i_\mathrm{m}(x_N, t) || \mathbf{d}_N ||
\end{pmatrix} ~,
\end{equation}
under the assumption that the transmembrane current density $i_\mathrm{m}(x_j, t)$ is constant along the compartment axis. 
Here, $||\mathbf{d}_j ||$ denotes the norm of $\mathbf{d}_j$. 
Note that $\mathcal{I}_\mathrm{a}(t)$ is a 4D tensor where the different axes correspond to $(x_j, y_j, z_j, t)$ respectively, 
while $\mathbf{I}_\mathrm{m}(t)$ is a 2D matrix where the different axes correspond to $(j, t)$ for $j\in\{1, N\}$.

%%%%%%%%%%%

\subsubsection{Application of linear volume conductor models}
