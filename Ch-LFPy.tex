...
So far, we have showed by example that a linear approximation can well explain the extracellular potentials (and by extension magnetic signals) from recurrently connected networks of multicompartment neuron models.
For the purpose of relating activity in point-neuron networks in particular to extracellular electric or magnetic measures of activity, however,
a number of approximations has been proposed and utilized in the literature. 
...
Using a computational model setup reminiscent of the hybrid scheme applied to a cortical microcircuit model in \cite**{Hagen2016},   
\cite**{Mazzoni2015} compared different LFP proxies (short for approximations) that could be extracted in two-population spiking point-neuron network models. 
These proxies, which has been used to mimic the LFP, EEG or similar in past studies, 
...
The LFP signal was also computed at different depths, 
...
Each proxy could be separated in depth dependent and temporal component, that is, the approximation in each channel could be formulated as a scaling factor times the temporal component. 
...
which \texit{a priori} ignore additional linear contributions by intrinsic dendritic filtering and the electrostatic forward model in question. 
