...
In the original application of the hybrid scheme \cite**{Hagen2016} to a point-neuron network model representing a generic cortical microcircuit \cite**{Potjans2014} \ehtxt{illustrated in \Fref{fig:LFPy:hybridmodel}},
...
\subsubsection{Hybrid scheme -- a cookbook}
...
\ehtxt{We also show in \Fref{fig:LFPy_hybrid}B  the alternative approach, that is, let an equivalent recurrent point-neuron network generate spike sequences with similar statistics as the MC-neuron network in panel A.} 
...
Here, columns 2-5 all incorporate linearized synaptic currents, 
...
\begin{equation}
H_{vu}(\mathbf{r},\tau) \coloneq \mathbf{F}^{\langle j \rangle} \left[ \overline{I}_{\text{syn}YX} \left( \delta(t - \Delta_{vu}) \ast f_{YX} \ast G_{vu} \right)(\mathbf{r}, \tau) \right]~,
\end{equation}
