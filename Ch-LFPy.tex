...
\ehnote{Tenker jeg redifinerer denne til en funksjon ala $\Delta_{YX}(t) = \delta(t - d_{YX})$}
...
\item The sequence of spike times $s_i(t) = \sum_k \delta (t - t_k)$ \ehnote{$\{ t_\mathrm{s}^{\langle i \rangle} \}$} of each of each neuron $i \in X$ is recorded throughout the entire simulation interval $[0, T_\mathrm{sim} \rangle$. 
...
It's activation times are defined as the concatenation of sequences $\left(s_i \ast \delta(t - d_{YX})\right)(t)$ for the subset $i \subset X$ being presynaptic to neuron $j \subset Y$ at the original synapse site(s) located on the compartment.  
\end{enumerate}

\ehnote{Jeg er fristet til aa sette opp en rekke simuleringer som viser effekten av hvert steg som proof of principle, liknende det Steinn setter opp i sin master}
...
may then be combined into a spatiotemporal filter kernel $H_{ji}(\mathbf{r},\tau)$ where $\tau$ denote time lag relative to presynaptic spike times in $s_i(t)$ \cite**{Hagen2016}. 
One may compute this filter as 
\begin{equation}
H_{ji}(\mathbf{r},\tau) \coloneq \mathbf{F}^{\langle j \rangle} \left[ \overline{i}_{\mathrm{syn}YX} \left( \Delta_{YX} \ast f_{YX} \ast G_{ji} \right)(\mathbf{r}, \tau) \right]~, 
\end{equation}
where $\Delta_{YX}(t) \coloneq \delta(t - d_{YX})$ and
the Green's like functions $G_{ji}(\mathbf{r}, \tau)$ encompass all dendritic filtering effects on the synaptic input currents into each corresponding distribution of transmembrane currents $\mathbf{I}_\mathrm{m}^{\langle ij \rangle}(\mathbf{r}, \tau)$.
...
\begin{equation}
\phi(\mathbf{r}, t) = \sum_{i \in X} \sum_{j \subset Y} \left(s_i \ast H_{ji} \right)(\mathbf{r}, t)~.
\end{equation}
In large networks however, 
computing the full set $H_{ji}(\mathbf{r},\tau)$ can become intractable as the number of connections (and consequently number of kernels) grow proportionally to the product $N_X N_Y$ if the connection probabilities are kept fixed. 
...
\begin{equation}
\hat{H}_{YX}(\mathbf{r}, \tau) = \frac{1}{N_X} \sum_{i \in X} \sum_{j \subset Y} H_{ji}(\mathbf{r}, \tau)~. 
\end{equation}
...
further successive steps of simplification may be applied to the point-neuron networks in order to describe neuronal dynamics at the level of populations. 
...
Here, we will not delve into much mathematical detail as the body of literature on both rate models and neural fields and their derivation is vast, 
...
\begin{equation}
\tau_\mathrm{m} \frac{dA(t)}{dt} = - A(t) + g_\sigma( I(t) ) ~,
\end{equation}
...
is how the hybrid forward modeling scheme and the above `kernel trick' may be adapted for use with population rate models in a self-consistent manner. 
...
\item The rate model allows for specifying key neuron and connectivity parameters such as 
...
Let us for now assume that these parameters are used to derive parameters and variables actually entering the rate equations.
...
\end{eqnarray}
Choices for filter be (but is not limited to) a boxcar, Gaussian or Hamming type windowing function symmetric around $\tau = 0$.   
...
\end{equation}
However, such an approach implies that we must perform a series of very computationally demanding calculations in order to compute transfer functions averaged over many thousands of multicompartment neurons. 
