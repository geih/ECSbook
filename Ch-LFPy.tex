...
\begin{equation}
\iiint_\Omega \rho_{\mathrm{syn}YX}(\mathbf{r}, \tau) d\Omega = k_{YX} \overline{i}_{\mathrm{syn}YX} f_{YX}(\tau) ~.
\end{equation}
Here, $\Omega$ denotes the synaptic input domain, that is, the neuron  membrane hence the unit of $\rho_{\mathrm{syn}YX}(\mathbf{r}, \tau)$ is proportional to \si{\ampere\per\metre^2}. 
...
With this quantity one may finally define the per-compartment synaptic input current density as 
\begin{equation}
i_{\mathrm{syn}YX}(\mathbf{r}_k, \tau) = \langle k_{\mathrm{syn}YX}[k] \rangle \overline{i}_{\mathrm{syn}YX} f_{YX}(\tau)
\end{equation}
scaled by $N_X$?
