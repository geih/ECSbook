...
So far, we have showed by example that a linear approximation can well explain the extracellular potentials (and by extension magnetic signals) from populations of multicompartment neuron models.
For the purpose of relating activity in point-neuron networks in particular to extracellular electric or magnetic measures of activity, 
a number of approximations has been proposed and utilized in the literature. 
As \textit{they for the most part are shite}, we will not go into much detail here, 
...
These proxies, which some times has been used to mimic the LFP, EEG or similar in past studies, 
included the population-averaged firing rate $\sum_X \nu_X(t)$, 
...
the absolute of the summed synaptic currents, 
and finally a weighted sum of the mean-subtracted summed synaptic currents with added delays along the time axis.
 
The task at hand was identifying which proxy maximises the fraction of variance explained ($R^2$) between the $z$-scored respective proxy predictions and ground-truth LFP signal. 
