...
$\widetilde{k}_{YX}$ & \( \displaystyle \mathcal{N}(2, 0.5)~\text{for}~X=E \\ \mathcal{N}(5, 1)~\text{for}~X=I \) & Multapse distribution function \\
$\overline{G}_{\mathrm{syn}YX}$ & \( \displaystyle \sim \begin{cases} \mathcal{N}(0.2\si{\nano\siemens}, 0.02\si{\nano\siemens})~\text{for}~X=E \\ \mathcal{N}(4\si{\nano\siemens}, 0.4\si{\nano\siemens})~\text{for}~X=Y \end{cases} \) & Maximum synaptic conductance \\
$E_{\mathrm{syn}X}$ & $\{0, -80\si{\milli\volt}\}$ & Synaptic reversal potential \\
\hline
\hspace
\hline
Symbol & Value/definition & Description \\
\hline
$l_\mathrm{soma}, $ & & \\
$\overline{g}_\mathrm{Na}$ & & \\
$\overline{g}_\mathrm{K}$ & & \\
$\overline{g}_\mathrm{Ih}$ & & \\
\hline
\end{tabular}
\end{center}
