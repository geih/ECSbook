...
at this point could destroy every connection of the recurrently connected network,
...
\ehnote{jeg highlighter naa at quasi-active greier er uavhengig av $G_{eff}$ i pkt 3. OK? Implementasjonen (min) av $G_{eff}$ inkluderer heller ingen endring i $E_L$ som her (tror ikke det vil ha noe aa si).}

\item Remove active ion channels \ehtxt{omitting the above quasi-active linearization tricks} if their net contributions to the total transmembrane currents can be assumed to be miniscular around typical membrane voltage values. 
