...
%
\begin{enumerate}
\item Compute the electrical activity of neurons using a the multicompartment (MC) framework presented in \Fref{chap:Neuron}, assuming that transmembrane currents are independent on the effect that they have on the extracellular potential ($V_\mathrm{e}$).

\item Compute the extracellular potential $V_\mathrm{e}$ resulting from the neural transmembrane currents computed in Step 1, using Volume Conductor (VC) theory (\Fref{chap:VC}-\ref{chap:Sigma}).
\end{enumerate}

The MC+VC scheme is not self-consistent, since Step 2 computes variations in $V_\mathrm{e}$ due to transmembrane currents, 
which were computed in Step 1, under the simplifying assumption that they do not affect $V_\mathrm{e}$. 
Hence, this scheme does not account for the feedback-effect from membrane-current induced variations in $V_\mathrm{e}$ on membrane currents.
...
There are, however, more complete and consistent schemes that account for both the ephaptic effects from $V_\mathrm{e}$  on neurodynamics as well as for effects of ion concentration dynamics on extracelluar potentials. 
...
These entering and leaving currents are in turn determined by a set of mechanisms that jointly determine the transmembrane current $I_\mathrm{m}$. 
...
while the transmembrane current $i_\mathrm{m}(x, t)$ is per unit length (\si{\ampere\per\metre}).
...
Keep in mind that the neural simulation software returns the midpoint potential $V_\mathrm{m}(x_j, t)$ for every $j \in \{1, \cdots, N \}$ compartment. 
...
\begin{equation}
V_\mathrm{m}(x_j+\frac{L_j}{2}, t) = \frac{\sum_{n=0}^N V_\mathrm{m}(x_{j+n}, t) / R_{j+n}}{\sum_{n=0}^N 1 / R_{j+n}}~.
...
and display some predictions using the MC+VC scheme. 
...
\item Let $k_{uv} \sim \widetilde{k}_{YX}$ denote the number of synapses per connection if a connection exist between neurons $u$ and $v$. 
...
$V_\mathrm{m}(t)$ denotes the postsynaptic membrane potential and 
$E_{\mathrm{syn}X}$ denotes the reversal potential of the synapse which is determined by the presynaptic cell type (i.e., excitatory or inhibitory).
For simplicity we will assume that $\overline{G}_{\mathrm{syn}YX}$ is independent of position.
...
Weights can, however, be drawn from a distribution with probability density function $\widetilde{G}_{\mathrm{syn}YX}$
...
\item Each postsynaptic neuron $v \in Y$ is modeled using the  `standard' MC neuron formalism such that their transmembrane currents $\mathbf{I}_\mathrm{m}^{\langle v \rangle}(t)$ or axial currents $\mathbb{I}_\mathrm{a}^{\langle v \rangle}(t)$ can be computed.
...
\begin{table}[!htp]
\caption{MC neuron and recurrent network parameters (continued in \Fref{tab:Ch-LFPy:MC-network-II}).
...
$\widetilde{\mathbf{r}}_X$ & $\propto \mathcal{N}(0, 100\si{\micro\metre})(z) \ast r^2~\text{for}~r\in[0, 100\si{\micro\metre}]$ & Cell body probability density function \\
...
$L_\mathcal{S} $ & \(\displaystyle \begin{cases} &\{30, 1000, 200\}~\si{\micro\metre}~\text{for}~X=\text{E} \\ &\{30, 1000, 200\}~\si{\micro\metre}~\text{for}~X=\text{I} \end{cases} \)  & Section length \\
$d_\mathcal{S} $ & \(\displaystyle \begin{cases} &\{30, 3, 2\}~\si{\micro\metre}~\text{for}~X=\text{E} \\ &\{15, 2, 2\}~\si{\micro\metre}~\text{for}~X=\text{I} \end{cases} \)  & Section diameter \\
$n_\text{seg}^\mathcal{S} $ & \(\displaystyle \begin{cases} &\{1, 21, 5\}~\text{for}~X=\text{E} \\ & \{1, 5, 5\} ~\text{for}~X=\text{I}  \end{cases} \) & \# of segments per section \\
\hline
...
$\overline{g}_\mathrm{Na}^\mathcal{S}$ & $\{0.12, 0.012, 0.012\}$ \si{\siemens\per\square\centi\metre}& Na$^+$  conductance \\
$E_\text{Na}$ & \SI{50}{\milli\volt} & Na$^+$ reversal potential \\
$\overline{g}_\mathrm{K}^\mathcal{S}$ & $\{0.036, 0.0036, 0.0036\}$ \si{\siemens\per\square\centi\metre} & K$^+$ conductance  \\
$E_\text{K}$ & \SI{-77}{\milli\volt} & K$^+$ reversal potential \\
$\overline{g}_\mathrm{Ih}^\mathcal{S}$ & $\{0.002, 0.01, 0.01\}$ \si{\siemens\per\square\centi\metre} & Ih-current conductance \\
\hline
\end{tabular}
\end{center}
...
\begin{table}[!htp]
\caption{Synaptic parameters for recurrent network (continued from \Fref{tab:Ch-LFPy:MC-network-I}).}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Symbol & Value/definition & Description \\
\hline
$\overline{G}_{\mathrm{syn}YX}$ & \( \displaystyle \sim \begin{cases} \mathcal{N}(\SI{0.2}{\nano\siemens}, \SI{0.02}{\nano\siemens})\Theta(G_\mathrm{syn})~\text{for}~X=\text{E} \\ \mathcal{N}(\SI{4}{\nano\siemens}, \SI{0.4}{\nano\siemens})\Theta(0)~\text{for}~X=\text{I} \end{cases} \) & Maximum synaptic conductance \\
$E_{\mathrm{syn}X}$ & \SI{0}{\milli\volt} for $X=\text{E}$, \SI{-80}{\milli\volt} for $X=\text{I}$ & Synaptic reversal potential \\
...
\hline
$\overline{G}_\mathrm{syn,ext}$ & \( \SI{0.5}{\nano\siemens} \) & External synapse conductance \\
$E_{\mathrm{syn,ext}}$ & \SI{0}{\milli\volt} & Ext. synapse rev. potential \\
...
\begin{table}[!htp]
\caption{Measurement and simulation parameters for recurrent network (continued from \Fref{tab:Ch-LFPy:MC-network-II}).}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Symbol & Value/definition & Description \\
...
\hline
$\phi$ & $\{V_\mathrm{e}(\mathbf{r}, t), \mathbf{P}(t) \} $ & Predicted extracellular signals \\ 
...
\begin{equation}
G_{\mathrm{tot}}(t) = \overline{G}_\mathrm{L} + \sum_X \sum_{u \subset X} \overline{G}_{\mathrm{syn}YX} \left( f_{YX} \ast s_u \ast \delta(t-\Delta_{vu}) \right)(t)
\end{equation}
where $\overline{G}_\mathrm{L} \coloneq \overline{g}_\mathrm{L} A_w$ is the total leak conductance from the specific conductivity $\overline{g}_\mathrm{L}$ and compartment area $A_w$, 
...
\begin{equation}
G_\mathrm{eff} \coloneq \langle G_\mathrm{tot}(t) \rangle \approx \overline{G}_\mathrm{L} + \sum_X \sum_{u \subset X} \langle \nu_u \rangle \overline{G}_{\mathrm{syn}YX} \int_0^\infty f_{YX}(t) dt ~.
\end{equation}
\ehtxt{Then, the original $\overline{G}_\mathrm{L}$ value is replaced by $G_\mathrm{eff}$ on a per-compartment basis.}
...
$\overline{g}_\mathrm{w}$ is significant. 
%\ghnote{Jmf. punkt over - her kan man kanskje si if g is significant, or ignored if not?} 
Here, $E_\mathrm{w}$ is the channel reversal potential and
...
\begin{eqnarray}
c_\mathrm{m}\frac{\partial V_\mathrm{m}(t)}{\partial t} &=&
	- \overline{g}_\mathrm{L} \left( \gamma_\mathrm{R}
						  \left( V_\mathrm{m}(t) - \langle V_\mathrm{m}(t) \rangle \right)
						  + \eta m(t)  \right)
	+ I_\mathrm{a} ~\text{, where} \\
\gamma_\mathrm{R} &\coloneq& 1 + \frac{\overline{g}_\mathrm{w} \omega_\infty(\langle V_\mathrm{m}(t) \rangle)}{\overline{g}_\mathrm{L}} ~\text{, and} \\
\eta &\coloneq& \frac{\overline{g}_\mathrm{w} ( \langle V_\mathrm{m}(t) \rangle - E_\mathrm{w} )}{\overline{g}_\mathrm{L}} \frac{\partial\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{\partial V_\mathrm{m}}~.
\end{eqnarray}
...
\item The leak conductance $E_\mathrm{L}$ is modified as
\begin{equation}
E_\mathrm{L}^\text{mod} = \langle V_\mathrm{m}(t) \rangle + \sum_\mathrm{w} \frac{\overline{g}_\mathrm{w} (\langle V_\mathrm{m}(t) \rangle - E_\mathrm{w})}{\overline{g}_\mathrm{L}} ~,
\end{equation}
...
by applying synaptic currents of $H_{\mathrm{syn}YX}(\tau)$ in a single MC neuron simulation for connections between populations $X$ and $Y$.
...
g_{V_\mathrm{m}}(t) &=& z_\text{score} \bigg(\sum_Y \frac{1}{N_Y} \sum_{v\in Y}V_\mathrm{m}^{\langle v \rangle}(t) \bigg) \\
...
\end{eqnarray}
%
Here, the function $z_\text{score}(g(t))$ ensures that each $g_\mathrm{proxy}(t)$ has zero mean and unit variance. 
...
Snapshots of the different temporal kernels $g_\mathrm{proxy}(t)$ derived from our original MC network are compared in \Fref{fig:g_proxy}A.  
...
As the $R^2$ score for different proxies may be affected by systematic phase shifts between the real signal and the different choices of $g_\mathrm{proxy}(t)$, 
...
%In \Fref{fig:g_proxy}A different temporal components $g_\mathrm{proxy}(t)$ derived similar to \cite**{Mazzoni2015} but from the recurrent MC neuron network are compared. 
