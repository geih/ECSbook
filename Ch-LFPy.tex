...
%
\begin{eqnarray}
I_\mathrm{i}(x,t) &=& - \frac{1}{r_\mathrm{i}} \frac{\partial V_\mathrm{m}(x, t)}{\partial x} \text{~and} \\
I_\mathrm{m}(x, t) &=& - \frac{\partial I_\mathrm{i} (x, t)}{\partial x} = \frac{1}{r_\mathrm{i}} \frac{\partial^2 V_\mathrm{m}(x,t)}{\partial x^2}~,
...
%
\begin{eqnarray}
I_\mathrm{i}(x_j, t) &\overset{h>0}{\approx}& - \frac{1}{r_\mathrm{i}} \frac{V_\mathrm{m}(x_j+\frac{h}{2}, t) - V_\mathrm{m}(x_j-\frac{h}{2}, t)}{h} \nonumber \\
...
We next outline how the membrane potential can be estimated at the endpoints of each compartment $\in \{x_j-L_j/2, x_j+L_j/2\}$), 
...
%
\begin{eqnarray}
V_\mathrm{m}(x_j - L_j/2, t) &\equiv& V_\mathrm{m}(x_j, t)~\mathrm{or} \nonumber \\
...
% 
where the connecting node to mid point resistances $R_{j+n} = (4 R_\mathrm{a}/\pi d_{j+n}^2) / (L_{j+n} / 2) = 8R_\mathrm{a} / \pi d_{j+n}^2 L_{j+n}$. 
\ehnote{Check! Should these resistive values rather be impedances?}
...
Up to this point, we have provided the basic `machinery' needed in order to compute the axial- and transmembrane currents in an MC neuron model, 
...
but thankfully MC neuron modeling software such as NEURON provides native support for computing the transmembrane current (\si{\ampere}) and/or transmembrane current density (\si{\ampere\metre^2}). 
...
For now, we will only emphasize that typical extracellular measures of extracellular electric or even magnetic signals can be derived from the transmembrane currents alone. 
