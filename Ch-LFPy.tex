...
Their particular values are summarized in \Fref{tab:Ch-LFPy:MC-network}.
Then:
...
\begin{table}[!htp]
\caption{MC neuron and recurrent network parameters. 
...
\ehnote{Skal definere multapse dist som diskret fordeling. Legg til params for ext. input.}}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
...
$N_X \in \{N_E, N_I\}$ & $\{4096, 1024\}$ & Number of neurons in population \\
...
$\widetilde{k}_{YX}$ & \( \displaystyle \sim \begin{cases} \mathcal{N}(2, 0.5)~\text{for}~X=E \\ \mathcal{N}(5, 1)~\text{for}~X=I \end{cases} \) & Multapse distribution function \\
\hline
$\mathcal{S}$ & $\{\text{soma, apic, basal} \} $ & Morphology sections \\
$L_\mathcal{S} $ & \(\displaystyle \begin{cases} &\{30, 1000, 200\}~\si{\micro\metre}~\text{for}~X=E \\ &\{30, 1000, 200\}~\si{\micro\metre}~\text{for}~X=I \end{cases} \)  & Section length \\
$d_\mathcal{S} $ & \(\displaystyle \begin{cases} &\{30, 3, 2\}~\si{\micro\metre}~\text{for}~X=E \\ &\{15, 2, 2\}~\si{\micro\metre}~\text{for}~X=I \end{cases} \)  & Section diameter \\
$n_\text{seg}^\mathcal{S} $ & \(\displaystyle \begin{cases} &\{1, 21, 5\}~\text{for}~X=E \\ & \{1, 5, 5\} ~\text{for}~X=I  \end{cases} \) & \# segments per section \\
\hline
...
\hline
$\overline{G}_{\mathrm{syn}YX}$ & \( \displaystyle \sim \begin{cases} \mathcal{N}(\SI{0.2}{\nano\siemens}, \SI{0.02}{\nano\siemens})~\text{for}~X=E \\ \mathcal{N}(\SI{4}{\nano\siemens}, \SI{0.4}{\nano\siemens})~\text{for}~X=I \end{cases} \) & Maximum synaptic conductance \\
$E_{\mathrm{syn}X}$ & \SI{0}{\milli\volt} for $X=E$, \SI{-80}{\milli\volt} for $X=I$ & Synaptic reversal potential \\
$f_{YX}(t)$ & $\beta$-function (\fref{eq:Ch-Neuron-Beta-function}) & Synaptic temporal kernel \\
$\tau_1$ & \SI{0.2}{\milli\second} for $X=E$, \SI{0.1}{\milli\second} for $X=I$ & Synaptic rise time constant \\
$\tau_2$ & \SI{1.8}{\milli\second} for $X=E$, \SI{9.0}{\milli\second} for $X=I$ & Synaptic decay time constant \\
\hline
$\mathcal{L}_{YXL}(z)$ & 
\(
	\begin{cases}
		& \frac{\mathcal{N}(0, 100)(z)}{3} + \frac{2\mathcal{N}(500, 100)(z)}{3}, \\
		& \quad X=E, Y=E, \mathcal{S} \setminus \{\text{soma}\} \\
		& \mathcal{N}(50, 100)(z), X=E, Y=I, \mathcal{S} \setminus \{\text{soma}\} \\
		& \mathcal{N}(-50, 100)(z), X=I, Y=E\\
		& \mathcal{N}(-100, 100)(z), X=I, Y=I \\
	\end{cases}
\)
...
\ehtxt{With that in mind, we will approximate the different constituents of the model that may give \textit{non-linear} contributions to the extracellular signal via the following steps:}
\ehnote{@GH: OK?}
