...
\ehnote{noen vill ha sammenlikning av performance foer beskrivelsen. Er ikke overbevist om at det gir mening}\ehtxt{In \Fref{sec:Schemes:comparison} we compare between different methods based on the MC+VC methodology that may be used for signal predictions.}
...
\ehtxt{Towards the end of this Chapter, we briefly introduce an alternative, and self-consistent framework (Section \ref{sec:VC:EMI}).}
...
\ehnote{har latt teksten i dette delkapitlet vaere. Dette flyttes tilbake igjen til slutten.}
...
({\bf A}) blabla
\ehnote{Geir's motiverende utkast for forenkla prediksjoner}}
\label{fig:LFPy_circuits}
\end{figure}
...
Synaptic activation times were determined by the spiking activity of a two-population point-neuron network model  with current-based \ehnote{dobbeltsjekk} synapses taken from a previous study \cite**{Cavallari2014}. 
...
the population-averaged membrane voltages of the point neurons
and the summed synaptic currents resolved into excitatory and inhibitory contributions onto different subpopulations.
...
\ehnote{Note that we throughout this Chapter allow the term ``spike'' to denote times of action potentials generated some neuron(s).} 
