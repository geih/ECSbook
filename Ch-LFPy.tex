...
Note that the transmembrane current $i_\mathrm{m}$ is here given in units of \si{\ampere\per\metre}.
...
\ehnote{saa kanskje jeg introduserer for/backward difference over allikevel da.}
...
but thankfully MC neuron modeling software such as NEURON and Arbor provides native support for computing the transmembrane current (with unit \si{\ampere}) and/or transmembrane current density per surface membrane area (with unit \si{\ampere/\metre^2}) for each compartment.
...
Note that $\mathbb{I}_\mathrm{a}(t)$ is a 3D tensor array with unit \si{\ampere \metre} and shape $(N, 3, N_t)$,
...
the set of axial currents $\mathbb{I}_\mathrm{a}$ which in the present context can be represented by a 3D tensor array with shape $(M, 3, N_t)$.
...
Conceptually, it is perhaps best to first consider a recurrently connected network of biophysically detailed neurons which allows for computing extracellular signals,
...
Connection probability may also depend on both spatial location and/or properties of the postsynaptic neurons such as compartment surface areas $\mathbf{A}_Y$.
\item Synaptic currents for each connection are given as $I_{\mathrm{syn}YX}(t)=\overline{G}_{\mathrm{syn}YX} f_{YX}(t)(V_\mathrm{m}(t)-E_{\mathrm{syn}Y})$
...
$V_\mathrm{m}(t)$ denotes the postsynaptic potential and $E_{\mathrm{syn}Y}$ denotes the reversal potential of the synapse which is determined by the presynaptic cell type (i.e., excitatory or inhibitory).
For simplicity we will assume that conductance magnitude is independent of $L$.
We will also assume that connection weights $\overline{G}_{\mathrm{syn}YX}$ are static, i.e., there are no synaptic plasticity rules in place.
\item The conduction delay $d_{vu}$ from presynaptic action potential generation time  to activation time of the synapse is greater than zero and drawn from some function $\Delta_{YX}(t)$.
For simplicity, we let conduction delays be independent of cell location and geometry.
(Other use cases may also pick delays from a statistical distribution or compute delays based on distance between pre- and post-synaptic units assuming a typical value for axonal action potential propagation speed.)
...
presynaptic populations may also represent point processes, external stimuli and similar,
which we will assume give approximately zero direct contributions to the signals predicted by the full network model.
...
\item The sequence of spike times $s_u(t) = \sum_k \delta (t - t_k)$ \ehnote{$\{ t_\mathrm{s}^{\langle u \rangle} \}$} of each of each neuron $u \in X$ is recorded throughout the entire simulation interval $[0, t_\mathrm{sim} \rangle$.
...
({\bf B}) Synapse counts per connection along the vertical $z$-axis in bins of \SI{20}{\micro\metre}.
({\bf C}) Network spike raster and mean population-averaged firing rates.
...
$\left(s_u(t) \ast \delta(t-d_{vu})\right)$
...
We may also relax the requirement that postsynaptic neurons in the hybrid scheme predict correct action potential times (or in fact \emph{any} action potentials or correct membrane potential statistics at all):
\begin{enumerate}
...
\begin{equation}
G_{\mathrm{tot}}(t) = G_L + \sum_X \sum_{u \subset X} \overline{G}_{\mathrm{syn}YX} \left( f_{YX} \ast s_u \ast \delta(t-d_{YX}) \right)(t)
\end{equation}
...
and $d_{YX}(t)$ the conduction delay.
...
and a normalized delay distribution ($\int_0^\infty \Delta_{YX}(t) dt \coloneq 1$),
the time-averaged effective conductance is approximately
\begin{equation}
G_\mathrm{eff} \coloneq \langle G_\mathrm{tot} \rangle \approx G_L + \sum_X \sum_{u \subset X} \langle \nu_u \rangle \overline{G}_{\mathrm{syn}YX} \int_0^\infty f_{YX}(t) dt ~.
\end{equation}
...
\item Active ion channel currents on the form
\begin{equation}
...
\mu &\coloneq& \frac{\overline{g}_\mathrm{w} ( \langle V_\mathrm{m}(t) \rangle - E_\mathrm{w} )}{g_\mathrm{L}} \frac{\partial\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{\partial V_\mathrm{m}}~.
\end{eqnarray}
...
while $\mu$ characterize whether the quasi-active current approximation act as positive $(\mu < 0)$ or negative $(\mu > 0)$ feedback.
%
For the special case $\mu=0$ the quasi-active current is `frozen', acting as a static, passive current \cite**{Ness2016}.
\ehnote{@TBN; hva mente du egentlig naar du skrev dette? Den kvasi-aktive stroembidraget er vel lineaert avhengig av $V_\mathrm{m}(t)$ hvis $\mu=0$ og foelgelig ikke statisk.}
%
...
\end{equation}
which ensure that the resting potential of the quasi-active model is similar to that of the fully active model.
...
This may reduce the accuracy of membrane voltage predictions, but more so at higher frequencies due to the intrinsic dendritic filtering.
...
It's activation times are defined as the concatenation of sequences $\left(s_u \ast \Delta_{YX} \right)(t)$ for the subset $u \subset X$ being presynaptic to neuron $v \subset Y$ at the original synapse site(s) located on  compartment $w$.
...
and a reasonable choice may be $\Delta t \lesssim \tau \in \{ \tau_\mathrm{m}, \tau_\mathrm{syn}, \ldots \}$,
that is, shorter than the typical time constants for membranes, synapses etc.
...
\end{center}
\caption{\textbf{Components for predictor of spike-signal filter kernels $H_{YX}(\mathbf{r}, \tau)$.}
...
{\bf D} Normalized distributions of transmission delays $\Delta_{YX} (\tau)$ for each connection relative to presynaptic activation time.
...
\label{sec:LFPy:kerneltricks}

The cable equation describing the membrane voltage dynamics across different neuron sections is already a linear partial differential equation.
...
as every component including synaptic conductances and voltage-gated ion channels of this hybridized simulation scheme is linearized and linearly separable.
...
\begin{equation}
H_{vu}(\mathbf{r},\tau) \coloneq \mathbf{F}^{\langle j \rangle} \left[ \overline{I}_{\mathrm{syn}YX} \left( \Delta_{YX} \ast f_{YX} \ast G_{vu} \right)(\mathbf{r}, \tau) \right]~,
\end{equation}
where $\Delta_{YX}(t) \coloneq \delta(t - d_{YX})$ and
the Green's like functions $G_{vu}(\mathbf{r}, \tau)$ encompass all dendritic filtering effects on the synaptic input currents into each corresponding distribution of transmembrane currents $\mathbf{I}_\mathrm{m}^{\langle ij \rangle}(\mathbf{r}, \tau)$.
If each kernel function which is specific to each pair of pre- and postsynaptic neuron can be computed,
one could compute the signal as the sum over convolutions between presynaptic spike times and corresponding kernels
\begin{equation}
\phi(\mathbf{r}, t) = \sum_{u \in X} \sum_{v \subset Y} \left(s_u \ast H_{vu} \right)(\mathbf{r}, t)~.
\end{equation}
In large networks however,
computing the full set $H_{vu}(\mathbf{r},\tau)$ can become intractable as the number of connections (and consequently number of kernels) grow proportionally to the product $N_X N_Y$ if the connection probabilities are kept fixed.
...
\begin{equation}
\hat{H}_{YX}(\mathbf{r}, \tau) = \frac{1}{N_X} \sum_{u \in X} \sum_{j \subset Y} H_{vu}(\mathbf{r}, \tau)~.
\end{equation}
...
by construction $H_{YX}(\mathbf{r}, \tau) \coloneq 0$ for $\tau < 0$ as any contribution to the signal $\phi(\mathbf{r}, t)$ is solely due to postsynaptic contributions which in this scenario can not occur before the presynaptic spike event at $\tau=0$.
...
\begin{equation}
\hat{\phi}(\mathbf{r}, t) = \sum_X \sum_Y \left(s_X \ast \hat{H}_{YX} \right)(\mathbf{r}, t)~.
\end{equation}
...
\subsubsection{\red{Population firing rate models}}
\label{sec:Schemes:populationmodels}
\index{Firing rate model}

Similar to how dynamics of large-scale multicompartment neuron networks as reflected by spikes of the individual neurons can some times be preserved in equivalent networks of few-compartment or point neurons after a series of simplifying steps,
...
and may explain a host of observed phenomena on the \emph{mesoscopic} (local populations, brain areas) and \emph{macroscopic} \ehnote{Ha!} (whole brain) level such as oscillations.
...
Let us for now assume that these parameters are used to derive parameters and variables entering the actual rate equations.
...
Conceptually this quantity is equivalent to the discrete population spike signal $s_X(t)$ convolved with some normalized temporal filter kernel or windowing function $w(t)$, that is:
\begin{eqnarray}
\nu_X(t) &\equiv& \frac{1}{N_X} \left(s_X \ast w\right)(t)~\text{, where} \\
...
Granted that we already computed population-averaged spatiotemporal kernels $\hat{H}_{YX}(\mathbf{r}, \tau)$ for the particular pre and postsynaptic network populations ($X, Y \subseteq X$) our rate model represents as above,
...
Can we deduce a methodology to compute the set of $\hat{H}_{YX}(\mathbf{r}, \tau)$ we need for all pre- and postsynaptic populations $X$ and $Y$ directly,
...
We can define the distribution of somas in space as the probability density function $\widetilde{r}_X$ introduced earlier. 
...
If we for instance only allow synaptic input within a certain depth interval $[z_1, z_2]$ where $z_1 < z_2$ (e.g., corresponding to a cortical layer),
...
\item Furthermore, we let the synaptic density be proportional to the membrane area of compartments $\mathbf{A}_Y$ weighted by $\mathcal{L}_{YXL}$.
Hence the expectation value for synaptic in-degree per compartment indexed by $k$ can be defined as
\begin{equation}
\langle k_{\mathrm{syn}YX}[k] \rangle \coloneq \frac{K_{YX}}{N_Y} \frac{ \mathcal{L}_{YXL}(\mathbf{r}[k]) \mathbf{A}_{Y}[k]}{\sum_k \mathcal{L}_{YXL}(\mathbf{r}[k]) \mathbf{A}_{Y}[k]} ~,
\end{equation}
where $\mathbf{r}[k]$ denotes the center location of each compartment.
...
\begin{equation}
H_{\mathrm{syn}YX}(\tau)[k] = N_X \langle k_{\mathrm{syn}YX}[k] \rangle \overline{I}_{\mathrm{syn}YX} \left( \Delta_{YX} \ast f_{YX}\right)(\tau) ~.
\end{equation}
...
$\widetilde{\mathbf{r}}_Y / \int_\Omega \widetilde{\mathbf{r}}_Y d\mathbf{r}$
...
\begin{equation}
f_{ij} = \frac{1}{4 \pi \sigma \int_\Omega \widetilde{\mathbf{r}}_Y) d\mathbf{r}}
	\int_\Omega \frac{d\mathbf{r}}{\Vert \mathbf{R}_i - \mathbf{r}_j - \mathbf{r}_\Omega \Vert}~, 
\end{equation}
where points $\mathbf{r}_\Omega \sim \widetilde{\mathbf{r}}_Y$. 
Analytical solutions for are however only possible to derive for certain source geometries, 
hence numerical integration methods will usually be required. 

\end{enumerate}


\section{\red{EH: Other approximations}}

So far, we have showed by example that a linear approximation can well explain the extracellular potentials (and by extension magnetic signals) from a recurrently connected network of multicompartment neuron models.
...
a number of heuristic signal approximations (``proxies'') has been proposed and utilized in the literature. 
As they for the most part are \textit{shite}, we will not go into much detail here, 
but emphasize that the aforementioned linearity leaves hope for \textit{some} of these approximations. 

Using a computational model setup reminiscent of the hybrid scheme applied to a cortical microcircuit model in \cite**{Hagen2016} outlined throughout \Fref{sec:LFPy:hybrid},   
\cite**{Mazzoni2015} compared different LFP \textit{proxies} (short for approximations) that could be extracted in two-population spiking point-neuron network models. 
...
the population-averaged somatic voltages of the point neurons $\sum_X V_\text{soma}^{\langle X \rangle}(t)$,
...
The LFP signal was predicted using the standard MC formalism in combination with the line-source volume conductor model, as described earlier. \ehnote{kryssref}
...
its performance is only marginally better as the signals in question reflect transmembrane currents which do not equate fluctuations the membrane voltages \ehnote{as discussed in XXXXX}. 
...
g_{\sum_I}(t) &=& z_\text{score}\bigg(  \sum_X \sum_Y K_{YX} \big( \nu_X \ast \Delta_{YX} \ast I_{\text{syn}YX})(t)  \bigg) \\
g_{\sum_{|I|}}(t) &=& z_\text{score}\bigg(  \sum_X \sum_Y \big| K_{YX} \big( \nu_X \ast \Delta_{YX} \ast I_{\text{syn}YX})(t) \big|  \bigg) \\
g_\text{WS}(t) &=& z_\text{score}\bigg(  \sum_X \sum_Y \big| \alpha_X K_{YX} \big( \nu_X \ast \Delta_{YX} \ast I_{\text{syn}YX} \ast \delta(-\tau_X) \big)(t) \big|  \bigg)~. 
\end{eqnarray}
%
...
while the term $\delta(-\tau_X)$ denotes a temporal delay. 
...
\end{equation}
%
In this formalism, the elements of each $f_\text{proxy}(\mathbf{r})$ and free parameters in $g_\mathrm{WS}(t)$ can then be trivially fitted by minimizing the mean squared difference between ground truth signal and proxy signal 
\begin{equation}
\text{MSE}_\text{proxy} = \langle \left(\phi_\text{GT}(\mathbf{r}, t) - \phi_\text{proxy}(\mathbf{r}, t)\right)^2 \rangle ~.
\end{equation}
%
...
As a proof of principle, we shall then compare these different LFP proxies against the ground truth LFP generated by our recurrently connected 
