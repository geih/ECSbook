...
\mu &:=& \frac{\overline{g}_\mathrm{w} ( \langle V_\mathrm{m}(t) \rangle - E_\mathrm{w} )}{g_\mathrm{L}} \frac{\partial\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{\partial V_\mathrm{m}}~.
\end{eqnarray}
Here, an equivalent gating variable was defined as
\begin{equation}
m(t) := \left( \omega(t) - \omega_\infty ( \langle V_\mathrm{m}(t) \rangle ) \right) / \frac{\partial\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{\partial V_\mathrm{m}}~,
\end{equation}
...
It's activation times are defined as the concatenation of sets $\{ t_\mathrm{s}^{\langle i \rangle} + d_{YX} \}$ for the subset $i \subset X$ being presynaptic to neuron $j \subset Y$ at the original synapse site(s) located on the compartment.  
\end{enumerate}
