...
\ehnote{Maa ha annen notasjon for celler og compartment index f.eks. $u, v, w$!}
...
where $\overline{G}_{\mathrm{syn}YX}$ is the maximum synaptic conductance and $f_{YX}(t) \in [0, 1]$ is one of the temporal kernels listed in \Fref{sec:Ch-Neuron:conductance-based-synapses}. 
...
We will also assume that connection weights $\overline{g}_{YX}$ are static, i.e., there are no synaptic plasticity rules in place. 
\item The conduction delay from presynaptic action potential generation time  to activation time of the synapse $d_{YX}$ is constant (i.e., independent of cell location and geometry for simplicity) and positive. 
We let $\Delta_{YX}(t) \coloneq \delta(t - d_{YX})$ define the corresponding delay function.
...
$\left(s_u \ast \Delta_{YX}\right)(t)$ 
...
\ehnote{legg til modifisert $E_L$!}
...
This may reduce the accuracy of membrane voltage predictions, but more so at higher frequencies. 
