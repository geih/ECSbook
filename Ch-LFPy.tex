...
There \ehtxt{are however \sout{exists}} more complete and consistent schemes that account for both the ephaptic effects from $\phi$ on neurodynamics as well as for effects of ion concentration dynamics on neuronal activity of extracelluar potentials. In general, these more complete schemes require numerical simulations of both the neural and extracellular dynamics using finite element or finite difference methods, and are very computationally expensive.

In most scenarios, ephaptic effects from extracellular potentials on neurodynamics \ehtxt{can be assumed to be \sout{are}} small, 
and ion concentrations tend to stay close to baseline. 
\ehtxt{Thus \sout{When this is true}}, the simplifying approximations applied in the two-step scheme do not critically affect the accuracy of its predictions. As the MC+VC scheme is far more computationally efficient than any of the more complete schemes, 
it remains the gold standard for computing \ehtxt{extracellular electric signals from \sout{$\phi$ in}} large neuronal population models mimicking physiologically realistic scenarios. 
...
When aiming to make \ehtxt{biophysically detailed \sout{realistic}} simulations of extracellular potentials, one might need to simulate large networks containing thousands of neurons. This is computationally demanding, even on an efficient scheme like MC+VC.
...
Tuning your model with billions of parameters into a reasonable regime is a bigger problem.
}
As we showed in \Fref{chap:VC},
VC theory \ehtxt{provides linear \sout{gave us}} analytical expression\ehtxt{s} for $\phi$ as a direct function of the neural current sources (Step 2), meaning that it is \ehtxt{usually} the simulations of the neurodynamics (Step 1) which is the bottleneck in the simulation. 
...
by construction $H_{YX}(\mathbf{r}, \tau) \coloneq 0$ for $\tau < 0$ as any contribution to the signal $\phi(\mathbf{r}, t)$ is solely postsynaptic which in this scenario can not occur before the presynaptic spike event at $\tau=0$.
...
For signal predictions set up either using spatiotemporal filter kernels for each recurrent connection from the hybrid scheme ($\langle H_{YX}(\mathbf{r}, \tau)$ in \Fref{fig:LFPy_kernels}) or deterministic rate-model version  ($\hat{H}_{YX}(\mathbf{r}, \tau)$ \Fref{fig:LFPy_kernel_predictor}), 
...
g_\text{WS}(t) &=& z_\text{score}\bigg(  \sum_X \sum_Y \big| \alpha_X K_{YX} \big( \nu_X \ast \widetilde{\Delta}_{YX} \ast I_{\text{syn}YX} \ast \delta(-\tau_X) \big)(t) \big|  \bigg)~. 
\end{eqnarray}
%
