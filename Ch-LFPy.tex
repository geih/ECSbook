...
\item Synaptic currents for each connection are given as $i_{\mathrm{syn}YX}(t)=\overline{g}_{\mathrm{syn}YX} f(t)(V_\mathrm{m}(t)-E_{\mathrm{syn}Y}$
...
For the purpose of predicting extracellular signals, 
...
\begin{eqnarray}
i_\mathrm{syn}(t) 
	&=& \overline{g}_\mathrm{syn}f(t)\left(V_\mathrm{m}(t) - E_\mathrm{syn}\right) \\
	&\approx& \overline{g}_\mathrm{syn}f(t)\left(\langle V_\mathrm{m}(t) \rangle - E_\mathrm{syn}\right) \\
	&=& \overline{i}_\mathrm{syn}f(t)~, 
\end{eqnarray}
where $\langle V_\mathrm{m}(t) \rangle$ denotes the spatiotemporally averaged postsynaptic potential.
\item Remove active ion channels if their net contributions to the total transmembrane currents are small.
...
\begin{equation}
\tau_\mathrm{w}(V_\mathrm{m}(t)) \frac{d\omega(t)}{dt} = \omega_\infty(V_\mathrm{m}(t)) - \omega(t)~.
\label{eq:Ch-LFPy:domega_dt}
\end{equation}
One can obtain the so-called \emph{quasi-active approximation} by linearizing each voltage dependent term around the steady state value 
$\langle V_\mathrm{m}(t) \rangle$. 
\Fref{eq:Ch-LFPy:active} may then be rewritten as the linear function 
\begin{eqnarray}
I_\mathrm{w}(t) \approx - \overline{g}_\mathrm{w} \hat{\omega}(t)\left( V_\mathrm{m}(t) - E_\mathrm{w} \right)~\text{, where} \\
\tau_\mathrm{w}(\langle V_\mathrm{m}(t) \rangle)  \frac{d\hat{\omega}(t)}{dt} = \omega_\infty( \langle V_\mathrm{m}(t) \rangle ) - \hat{\omega}(t)
\end{eqnarray}
This effectively turns the equivalent circuit of each compartment into an RLC type circuit. 
\ehnote{For `compactness' kan vi beholde versjonen over - ikke fruktelig interressant aa differensiere mellom regenerative eller restorative stroemmer i denne konteksten? Har jeg forstaatt ting rett, vil ogsaa forskjellige bidrag summere lineaert - hvordan ser dette ut?}

Alternatively, one may introduce the equivalent gating variable
\begin{equation}
m(t) := \left( \omega(t) - \omega_\infty ( \langle V_\mathrm{m}(t) \rangle ) \right) / \frac{d\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{dV_\mathrm{m}}
\end{equation}
and rewrite the dynamics of the compartment as
\begin{eqnarray}
c_\mathrm{m}\frac{dV_\mathrm{m}(t)}{dt} &=& 
	- g_\mathrm{L} \left( \gamma_\mathrm{R}  
						  \left( V_\mathrm{m}(t) - \langle V_\mathrm{m}(t) \rangle \right)
						  + \mu m(t)  \right)
	+ I_\mathrm{axial} + \cdots ~\text{, where} \\
\gamma_\mathrm{R} &:=& 1 + \overline{g}_\mathrm{w} \omega_\infty(\langle V_\mathrm{m}(t) \rangle) / g_\mathrm{L} ~\text{, and} \\
\mu &:=& \frac{\overline{g}_\mathrm{w} ( \langle V_\mathrm{m}(t) \rangle - E_\mathrm{w} )}{g_\mathrm{L}} \frac{d\omega_\infty (\langle V_\mathrm{m}(t) \rangle)}{dV_\mathrm{m}}~.
\end{eqnarray}
Here, $\gamma_\mathrm{R}$ denotes the ratio between the total and leak conductance 
while $\mu$ characterize whether the quasi-active current approximation act as positive $(\mu < 0)$ or negative $(\mu > 0)$ feedback.  
The linear dynamics of the equivalent gating variable is governed by
\begin{equation}
\tau_\mathrm{w} (\langle V_\mathrm{m}(t) \rangle) \frac{dm(t)}{dt} = V_\mathrm{m}(t) - \langle V_\mathrm{m}(t) \rangle - m(t)~. 
\end{equation}

\item For the special case $\mu=0$ the quasi-active current is `frozen', acting as a static, passive current \cite**{Ness2016}. 
\ehnote{@TBN; hva mente du egentlig naar du skrev dette? Den kvasi-aktive stroembidraget er lineaert avhengig av $V_\mathrm{m}(t)$ hvis $\mu=0$ og foelgelig ikke statisk.}
...
\item Further reduction of number of state variables describing synapses can be achieved by having at most one synapse per connection type (from $X$ onto $Y$) per compartment. 
It's activation times are defined as the concatenation of $\{ t_\mathrm{s}^{\langle i \rangle} + d_{YX} \}$ for the subset $i \subset X$ being presynaptic to neuron $j \subset Y$ at the original synapse site(s) located on the compartment.  
\end{enumerate}
