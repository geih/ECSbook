...
The naïve computational neuroscientist should remind her/himself that this assumption is likely to be violated if the spatial discretization chosen for the model neuron(s) is too coarse. 
%
...
Note that $\mathcal{I}_\mathrm{a}(t)$ is a 4D tensor with unit \si{\ampere \metre} where the different axes correspond to $(x_j, y_j, z_j, t)$ respectively, 
while $\mathbf{I}_\mathrm{m}(t)$ is a 2D matrix with unit \si{A} where the different axes correspond to $(j, t)$ for $j\in\{1, N\}$. 
...
As derived from \textit{volume conductor theory} \ehnote{crossref Chap. 4}, 
...
which models neuronal sources \textit{and} measurement sites as infinitesimally small points a conductive medium \ehnote{add crossref}. 
...
\end{equation}
% 
The full linear system may be written out as
% 
\begin{equation}
\begin{bmatrix}
\phi(\mathbf{R}_1, t) \\
