...
the set of axial currents $\mathcal{I}_\mathrm{a}$ which in the present context is represented by a 3D tensor array with shape $(3, N, N_t)$\ehnote{?}. 
...
thus the elements $f_{ij}$ are of shape $3 \times 3$. 

which in the next step may be multiplied with the corresponding axial current magnitudes $[I_\mathrm{a}(\mathbf{r}_j, t)]$. 
The magnetic permeability of tissue is approximately equal to  $\mu_0=$ 
\ehnote{need to doublecheck some details}
