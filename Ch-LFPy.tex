...
\item Let $\mathbf{r}_u \in \Omega_X$ denote the discrete somatic location of neuron $u$ in a spatial domain $\Omega_X$.
Alternatively, we may let a function $\rho_X(\mathbf{r})$ describe the density of somatic locations of population $X$ in space.
\ehnote{Det siste git kanskje mest mening}
...
(Other use cases may also pick delays from a statistical distribution or compute delays based on distance between pre- and post-synaptic units.)
\item Each postsynaptic neuron $v \in Y$ is modeled using the  `standard' multicompartment neuron formalism such that their transmembrane currents $\mathbf{I}_\mathrm{m}^{\langle v \rangle}(t)$ or axial currents $\mathcal{I}_\mathrm{a}^{\langle v \rangle}(t)$ can be computed.
...
We may choose to relax this requirement if a population $X$ represents an external population feeding persistent, uncorrelated events with `flat' spiking statistics (e.g., fixed-rate Poisson point processes) into the recurrently connected network.
...
Outgoing colored lines from each ball-and-stick neuron model denotes a connection from the soma of presynaptic neurons to connections distributed on postsynaptic neurons.
...
Assuming a fixed average presynaptic spike rate $\langle \nu_u \rangle \coloneq \Delta t^{-1} \int_t^{t+\Delta t} s_u(t) dt$,
a normalized delay distribution ($\int_0^\infty \Delta_{YX}(t) dt \coloneq 1$),
...
\end{equation}
may be approximated by a linearly dependent current \cite**{Remme_2010,Ness2016}
...
but generalize also to current types with more than one gating variable (e.g., K-currents), see \cite**{Remme_2010} for details.

\item The leak conductance $E_L$ is modified as
\begin{equation}
E_L^\text{mod} = \langle V_\mathrm{m}(t) \rangle + \sum_w \frac{g_w (\langle V_\mathrm{m}(t) \rangle - E_w)}{g_L} ~,
\end{equation}
...
It's activation times are defined as the concatenation of sequences $\left(s_u \ast \Delta_{YX} \right)(t)$ for the subset $u \subset X$ being presynaptic to neuron $v \subset Y$ at the original synapse site(s) located on the compartment.
...
and a reasonable choice may be $\Delta t \lesssim \{ \tau_\mathrm{m}, \tau_\mathrm{syn}, \ldots \}$,
that is, shorter than the typical time constants for membranes, synapses etc.

\end{enumerate}

\ehnote{Jeg setter opp en rekke simuleringer som viser effekten av hvert steg som proof of principle, liknende det Steinn setter opp i sin master, ogsaa for visualiseringer av hvert steg}
...
Functions $\langle H_{YX}(\mathbf{r}, \tau) \rangle$ for each connection between every possible pre- and postsynaptic network population $X$ and $Y$, respectively.
...
{\bf C} Densities of synapses for each connection between the two populations.
{\bf D} Normalized continuous density functions of distributions of transmission delays $\Delta_{YX}(\tau)$ for each connection relative to presynaptic activation time.
...
Points 1--4 above allows for computing a \emph{linear} relation between spikes of each presynaptic neurons and resulting contribution to the extracellular signal(s),
...
\begin{equation}
\phi(\mathbf{r}, t) = \sum_{v \in Y} \sum_{v \subset Y} \left(s_u \ast H_{vu} \right)(\mathbf{r}, t)~.
\end{equation}
...
\cite**{Hagen2016} showed however that a good approximation to the signal $\phi(\mathbf{r}, t)$ could be obtained by estimating kernels averaged over all pre- and postsynaptic neurons in each population $X$ and $Y$ defined as
\begin{equation}
\hat{H}_{YX}(\mathbf{r}, \tau) = \frac{1}{N_X} \sum_{u \in X} \sum_{j \subset Y} H_{vu}(\mathbf{r}, \tau)~.
\end{equation}
In order to compute these kernels directly,
...
by construction $H_{YX}(\mathbf{r}, \tau) \coloneq 0$ for $\tau < 0$ as any contribution to the signal $\phi(\mathbf{r}, t)$ is solely due to postsynaptic contributions which can not occur before the presynaptic spike event in this scenario.
...
The measured kernels are essentially finite impulse responses (FIR) that must be 0 for all time lags $tau < \Delta t$.
...
Similar to how dynamics of large-scale multicompartment neuron networks as reflected by spikes of the individual neurons can some times be preserved in equivalent networks of point neurons after a series of simplifying steps,
...
Rate models may simply describe the mean activity of subsets of neurons (`populations') but may also explain activity both in space and time.
The latter is some times referred to as \emph{neural fields} and may readily produce observed phenomena such as traveling waves of activity.
Certain rate models may also describe activity of individual neurons.
...
Conceptually this quantity is equivalent to the discrete population spike signal $s_X(t)$ convolved with some normalized temporal filter kernel or windowing function $\mathcal{N}(t)$, that is:
\begin{eqnarray}
...
\item $\rho_{\mathrm{syn}YX}(\mathbf{r}, \tau)$ depends on some spatial specificity of synaptic connections \( \mathcal{L}_{YXL}(\mathbf{r}) \) and areas of putative target sections.
...
Furthermore, we let the synaptic density be proportional to the membrane area of compartments $\mathbf{a}_Y$ weighted by $\mathcal{L}_{YXL}$.
Hence the expectation value for synaptic in-degree per compartment indexed by $k$ can be defined to be
\begin{equation}
\langle k_{\mathrm{syn}YX[k]} \rangle \coloneq \frac{K_{YX}}{N_Y} \frac{ \mathcal{L}_{YXL}(\mathbf{r}_k) \mathbf{a}_{Yk}}{\sum_k \mathcal{L}_{YXL}(\mathbf{r}_k) \mathbf{a}_{Yk}} ~,
\end{equation}
