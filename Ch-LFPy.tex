...
As a proof of principle, we shall then compare these different LFP proxies against the ground truth LFP generated by our recurrently connected multicompartment network model. 
\ehnote{todo! If these proxies fail for this more-or-less self-consistent case, why should they be used for point neuron networks?!}
\Fref{fig:g_proxy} compares different temporal components $g_\mathrm{proxy}(t)$ derived similar to \cite**{Mazzoni2015} but from the recurrent multicompartment neuron network. 



\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figures/Ch-LFPy/Ch-LFPy-g_proxy.pdf}
\end{center}
\caption{\textbf{Comparison of temporal components $g_\mathrm{proxy}(t)$.} 
Normalized temporal components of different candidate proxy signals for mimicking extracellular signals from recurrent network model. 
\ehnote{kommer mer her.}
}
\label{fig:g_proxy}
\end{figure}
