...
({\bf A}) Illustration of self-consistent neuronal network simulation where network activity where the primary output are spikes, 
...
Here, synaptic activation times are set from pre-recorded spike times recorded to a database. 
As the 
}
...
\begin{equation}
G_{\mathrm{eff}}(t) = G_L + \sum_X \sum_{u \subset X} \overline{G}_{\mathrm{syn}YX} \left( f_{YX} \ast s_u \ast \Delta_{YX} \right)(t)
\end{equation}
...
$\int_0^\infty f_{YX}(t) dt = F_{YX}$
the time-averaged effective conductance is approximately
\begin{equation}
\overline{G}_\mathrm{eff} \approx G_L + \sum_X \sum_{u \subset X} \langle \nu_u \rangle \overline{G}_{\mathrm{syn}YX} F_{YX} ~.
\end{equation}
...
Here, 
$\overline{g}_\mathrm{w}$ is the maximum conductance,
$E_\mathrm{w}$ its reversal potential and 
