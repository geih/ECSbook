...
can be expressed as a sum of transmembrane current sources, 
i.e., as a current source density (CSD), like in \fref{eq:VC:CSD1}:
...
by measuring gradients in $V\text{e}$ resulting from a controlled current injection into the tissue.
...
and ohmic model for the tissue current, and a measured value for $\sigma_\text{t}$, 
...
V_\mathrm{e}({\bf r}) \approx \frac{1}{4 \pi \sigma_sigma_\mathrm{t}t} \frac{|{\bf p}| \cos \theta}{|{\bf r} - {\bf r}_c|^2},
\end{equation}
