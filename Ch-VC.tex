...
\snnote{Har provd aa bytte ut $\phi$ med $V$}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We start by deriving the contribution to $V$ from a single neuronal point source $I_k$ at the point ${\bf r_k}=0$ (\fref{fig:VC:pointsource}{\bf A}).
...
\sntxt{\sout{Here, we could compute $V$ by solving \fref{eq:VC:CSD3}. However, due to the spherical symmetry of this simple case, we can take a shorter path by realizing that the current density (\fref{eq:VC:ohmici}) must} In this simple case, we know that the current density is} radially directed, and that its magnitude must be solely a function of distance $r$ from the source:


\begin{equation}
i_t({\bf r}) = i_t(r) = -\sigma_t \frac{dV(r)}{dr}.
\end{equation}

\snnote{Setningen under var ganske lang..hva med:}
\sntxt{\sout{Knowing this, we may find $V$ by simply noting that, s} S}ince there can be no charge accumulation anywhere inside the sphere, the net current $I_k$ injected into the center of the spherical volume with radius $r$ must equal the net current leaving through the surface of the volume \sntxt{.\sout{, which} The net current leaving the surface can in turn be found by multiplying \sout{in turn must equal}} the current density $i_t(r)$ at the surface \sntxt{\sout{multiplied}} with the surface area $4\pi r^2$. We then get the relationship:

\begin{equation}
I_k = -4\pi \sigma_t r^2  \frac{dV(r)}{dr} \, \iff \, \frac{dV(r)}{dr} = -\frac{I_k}{4\pi \sigma_t r^2 }.
\label{eq:VC:knut}
\end{equation}
...
To obtain the final solution for $V$, we integrate eq. \ref{eq:VC:knut} from $r$ to $\infty$:
\snnote{Forslag: (original kommentert ut)}

%\begin{equation}
%\int_r^{\infty} \frac{dV(r')}{dr'} dr' = \int_r^{\infty} -\frac{I_k}{4\pi \sigma_t r'^2 } dr'.
%\label{eq:VC:knut2}
%\end{equation}

\sntxt{
\begin{equation}
\int_{\infty}^r \frac{dV(r')}{dr'} dr' = \int_{\infty}^r -\frac{I_k}{4\pi \sigma_t r'^2 } dr'.
\label{eq:VC:knut2}
\end{equation}
}
Since $V({\infty}) = 0$, this leads to the final expression:
...
\begin{equation}
V({\bf r},\sntxt{\sout{t}})_k = \frac{I_k}{4\pi \sigma_t} \int \frac{dr_k}{|{\bf r-r_k}|} =
...
\begin{equation}
V({\bf r},\sntxt{\sout{{\bf t}}}) = \sum_k \frac{I_k}{4\pi \sigma_t \Delta s_k} \log \left| \frac{\sqrt{h_k^2+\rho_k^2}-h_k}{\sqrt{\ell_k^2+\rho_k^2}-\ell_k} \right|.
...
The line-source approximation can be expected to give a better prediction of $V$ than the point-source approximation at points in space that are very near neuronal membranes, especially when it comes to predicting rapid fluctuations in $V$, such as the extracellular action potential signature \cite**{Holt1999}. At points further away from membranes, the two approaches give converging predictions, see Fig.~\ref{fig:VC:point_vs_linesource}.
...
\begin{equation}
\nabla^2 V({\bf r}) = - \frac{C({\bf r})}{\sigma_t}
\label{eq:VC:CSD4}
\end{equation}
...
We already listed the solution to this in \fref{eq:VC:pointsource2}, but we here provide a more rigorous derivation. We start by integrating both sides of \fref{eq:VC:CSD5} over an arbitrary 3D volume \sntxt{$\Omega$} containing the source \sntxt{$I'$}:

\begin{equation}
\iiint_{\sntxt{\Omega}} \nabla^2V({\bf r}) \,d\sntxt{\Omega} =  - \frac{I'}{\sigma_t} \iiint_\sntxt{\Omega} \ \delta^3({\bf r-r'}) \, d\sntxt{\Omega},
\label{eq:VC:marit}
\end{equation}

By the definition of the delta-function, the right hand side of \fref{eq:VC:marit} is simply $I'/\sigma$. Using Gauss' theorem, we can convert the volume integral on the left hand side of \fref{eq:VC:marit} to \sntxt{an integral over the surface $S$ \sout{a surface integral}}, so that \fref{eq:VC:marit} becomes:

\begin{equation}
\oiint_{S} \nabla V({\bf r}) \cdot \, d{\bf S}  = - \frac{I'}{\sigma_t},
\label{eq:VC:berit1}
\end{equation}
\snnote{saann det staar under naa ser det jo egentlig ut som om nabla er en vektor. og det synes jeg egentlig er veldig fint. saa kan vi bare ta med retningen.}
To solve this, it is convenient to chose the volume that we integrate over to be a sphere centered at the source location ${\bf r'}$, and with radius $R = |{\bf r-r'}|$ (\fref{fig:VC:pointsource}{\bf B}). Due to the symmetry of problem, we then know that the electrical potential is the same for all ${\bf r}$ on the surface of the sphere, so that $V({\bf r}) = V(R)$. We also know that its gradient $\nabla V({\bf r}) = d V(R)/dR \sntxt{~{\bf e}_R}$ is constant over the surface, and perpendicular to the surface increment $d{\bf S} \sntxt{= dS {~\bf e}_R}$. If we use this, \fref{eq:VC:berit1} becomes:
\begin{equation}
\oiint_{S} \frac{d V(R)}{dR} d{S}  = - \frac{I'}{\sigma_t},
\label{eq:VC:berit1ogenhalv}
\end{equation}
which has the solution:
\begin{equation}
4\pi R^2 \frac{d V(R)}{dR} = \sntxt{-} \frac{I'}{\sigma_t}
\label{eq:VC:berit2}
\end{equation}
\snnote{synes det er enklere aa integrere fra $\infty$ til $R$, for a slippe unna litt fortegnsfiksing..kan vaere dette er hoyst subjektivt.}
If we integrate this from \sntxt{\sout{$R$ to $\infty$} $\infty$ to $R$}, and use that $V(\infty) = 0$, we get:
\snnote{trenger kanskje ikke $V({\bf r})$ her, siden dette er forklart lenger ned?}
\begin{equation}
V(R) = \sntxt{\sout{V({\bf r})}} = \frac{I'}{4\pi \sigma_t R}.
\label{eq:VC:berit3}
\end{equation}
We may now insert back for $V(R)= V({\bf r})$ and $R = |{\bf r-r'}|$ to obtain the desired Green's function:

\begin{equation}
V({\bf r})= \frac{I'}{4\pi \sigma_t |{\bf r-r'}|},
\label{eq:VC:berit4}
\end{equation}
As earlier stated, the solution for a general CSD can be expressed as a convolution over the Green's function, so that:

\begin{equation}
V({\bf r}) = \frac{1}{4\pi \sigma_t}\iiint_{\sntxt{\Omega}} \frac{C({\bf r'})}{|{\bf r}-{\bf r'}|} \,d\sntxt{\Omega},
\label{eq:VC:csds}
\end{equation}
...
\snnote{Hva med aa referere til likningene i dette          kapittelet, saa slipper man aa bla saa langt?}
Although \sntxt{\sout{\fref{eq:Basics:continuity1}} \fref{eq:VC:CSD1}} may seem intuitively reasonable, we introduced it above more or less as a postulate. A thorough derivation of this equation was given by \citeasnoun**{Gratiy2017}, and for the interested reader, we will here go through the main steps of this derivation. 
...
\snnote{Jeg synes dette er finfint!=)}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The \textit{dipole approximation} \index{Dipole approximation} is an alternative to \fref{eq:VC:pointsources} for predicting the extracellular potential. The approximation is good provided that the "measuring point" for $V$ is far away from the source-sink distribution that it originates from, like in the case of EEG.
...
\snnote{tenker at det kan vaere lurt aa bruke ${\bf r}$ her, for lettere aa kunne lese det i sammenheng med $\fref{eq:VC:csds}$? Men lurer paa om det ble litt rotete..}
\begin{equation}
\sntxt{V({\bf r}) =} V(R) = \frac{C_{monopole}}{R} + \frac{C_{dipole}}{R^2} + \frac{C_{quadrupole}}{R^3} + \frac{C_{octopole}}{R^4} + ...
\label{eq:VC:multipole}
\end{equation}
where $R \sntxt{= {\bf r} - {\bf r}_c}$ is the distance from the center of the source distribution \sntxt{(${\bf r}_c$)}, $C_{monopole}$ is the monopolar contribution of the CSD to $V$, $C_{dipole}$ is the dipolar contribution from the CSD to $V$, and so on. The derivation of this expression can be found in Appendix \ref{app:dipoleappendix}.

As we explained in \fref{sec:Neuron:membranecurrents}, the net sum of currents over a neuronal membrane is always zero, meaning that the monopolar contribution, i.e., first term in \fref{eq:VC:multipole}, vanishes. Furthermore, the quadrupole, octopole and higher-order contributions to $V$ decay rapidly with distance $R$. Provided that we are sufficiently far away from the source distribution, $V$ can therefore be well approximated by dipole contribution alone:

%\begin{equation}\label{eq:VC:CDA}
%V(\mathbf{R}) \approx \frac{1}{4 \pi \sigma_t} \frac{|\mathbf{p}| \cos \theta}{R^2},
%\end{equation}
%
%
%where we have expressed $C_{dipole}$ in terms of the current dipole moment ($\mathbf{p}$) and the angle ($\theta$) between the current dipole moment and the distance vector ($\mathbf{R}$), as explained in Appendix \ref{app:dipoleappendix}.
\sntxt{
\begin{equation}\label{eq:VC:CDA}
V({\bf r}) \approx \frac{1}{4 \pi \sigma_t} \frac{|{\bf p}| \cos \theta}{|{\bf r} - {\bf r}_c|^2},
\end{equation}
where we have expressed $C_{dipole}$ in terms of the current dipole moment ($\mathbf{p}$) and the angle ($\theta$) between the current dipole moment and the distance vector ($\mathbf{R} = {\bf r} - {\bf r}_c$), as explained in Appendix \ref{app:dipoleappendix}.
}
