...
Neural currents give rise to extracellular potentials,
and from a given configuration of current sources and sinks, we have demonstrated how we can use volume conductor theory to make predictions about the ensuing extracellular potentials at any point in space. However, in experiments these extracellular potentials must be measured through some kind of recording device, which might itself affect the measured potentials in several different ways, depending on both the measurement device and the extracellular potential that is being measured. 

For example, if the extracellular potential changes across the surface of the recording electrode, the measured potential will be a spatial average of the potential across the surface of the electrode \citep{Ness2015, Vermaas2020}\tvnnote{Bør finne lærebok-referanse til dette?}, which can introduce a spatial filtering effect. In practice, this means for example that extracellular action potentials will be less prominent on electrodes with diameters larger than $\sim$30~$\mu$m \tvnnote{Bør dette med, og skal vi i så fall lage enkel figur?}.
...
\subsubsection{\red{Point-electrode approximation}}
