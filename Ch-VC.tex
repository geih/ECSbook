...
and the reader may chose to skip this part, which is mainly about terminology. 
...
\end{equation}
the theory for dealing with the inverse problems is often based on decomposing the current (density)
...
\end{equation}
where (in both cases), the first term has to do with cellular activity
...
The problem with this definition is that it represents a "very fine grained" way of thinking.
There are essentially two ways by which $V_\mathrm{e}$ can change in tissue: 
...
However, whereas ${\bf i}_\mathrm{imp}$ is explicitly defined as membrane currents, 
${\bf i}_\mathrm{pri}$ can be defined in a rather non-binding way 
...
Regarding spatial scale, the CSD equation (\fref{eq:VC:CSDit}) is on the same
