...
A more sophisticated choice may be to assume that the transmembrane current is evenly distributed over the segment axis, a choice which is referred to as the line-source approximation \cite**{Holt1999,Linden2014}. The contribution to the extracellular potential from a current $I_k$ in a segment $k$ can be found analytically by integrating eq. \ref{eq:VC:pointsource} over the center-line axis of the compartment (see Appendix C in \cite**{Holt1998}). We here skip the derivation \tvnnote{Hadde jo kanskje gitt mening aa ha dette med i appendix? Litt rart aa maatte lete i appendikset til Holt sin gamle doktoravhandling, naar det er en formel vi bruker saapass mye.}, but give the final solution. For a segment with length $\Delta s_k$, the contribution to the extracellular potential will be:
