...
%(see \fref{sec:Eldiff})
\tvnnote{Men andre steder sier vi at denne loven er "constitutive"? Kanskje bare referere til "Note on Ohm's law" i Basics?}. 
\ghnote{Godt poeng. Jeg skrev at Ohms forutsetter kvasistatisk + null diffusjon, men tok det vekk. 
Greia er vel uansett at disse to i prinsippet kan (for hoeye frekvenser eller store gradienter) gjoere 
at Ohms lov blir en daarlig approkimasjon. AA referere til "Notes on Ohms lov" tror jeg ikke er saa relevant her.
Det handler jo mer om "nanoverden" versus "mikroverden". Jeg tenker at vi har troeblet nok med dette tidligere, 
og foreslaar at vi ikke sier annet enn at vi bruker Ohms lov.}
...
\tvnnote{Syntes vi burde faa det bedre frem at dette foreloebig ikke er noen approksimasjon}
\ghnote{ yttet rekkefoelge paa avsnitt. Bedre naa? Det er ingen approksimasjon, men forutsetter at vi er
utenfor skyen av kidler, sant? Torbjorn, hvis du er enig, fjern kommentarer.}
\begin{equation}
...
\tvntxt{\ghtxt{\sout{We start by ignoring}For now, we ignore} any possible temporal signal distortion introduced by the
recording equipment, but get back to this in \fref{sec:VC:elec_distrotions}.} \ghnote{Vet ikke om det bare er meg, men "we start by ignoring" hoertes rart ut. Ellers: Jeg har lest det groenne her, og synes det saa fint ut. Paint it black, og fjern kommentarer.}
...
\ghtxt{The effect of the electrode size can be accounted for through a trivial expansion of the 
point-electrode approximation: We calculate $V_\mathrm{e}$ at all points across the electrode surface 
and compute the average potential. This approach is referred to as the disc-electrode approximation 
...
\tvnnote{syntes det er fint aa ha med at dette egentlig bare er en triviell utvidelse av punk-kilde approksimasjonen} 
\ghnote{Sjekk versjonen over. Er den ok? Jeg reagerte paa at det tidligere sto "many points", 
og byttet det med "all points" Er ikke det riktige? Det er jo analytisk, og man integrerer opp? 
Eller tar man et sample?}
...
Efforts to model large non-conducting volumes such as this are \ghtxt{\sout{also}} 
...
\ghnote{Saa igjennom. Paint black og fjern kommentarer. Vurder aa fjerne den siste setningen. 
Er det viktig aa si dette her? Det er vel implisitt at naar vi sier "Effect of" saa mener vi effekten paa ekstracellulaere potensialer, 
og ikke paa andre ting, som for eksempel humoeret? }
...
\ghnote{Flyttet siteringene til etter "electrode surface", da sikkert ikke alle disse har kalt det for $V_\text{true}$.}
...
Studies of such phenomena require the use of DC amplifiers \cite**{herreras2016,harting2017}
...
\ghtxt{Apart from issues conserving the unobservable slow potential fluctuations}, 
...
\ghnote{Veldig bra. Paint it black.}
