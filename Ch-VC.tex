...
\label{eq:VC:ohmici}
\end{equation}
This assumption implies that \tvntxt{\sout{(i)}} we have made the quasi-static approximation of Maxwell's equations (see \fref{sec:Basics:Quasistatic}), and assume that diffusive currents in the extracellular space can be neglected
(see \fref{sec:Eldiff})\tvnnote{Men andre steder sier vi at denne loven er "constitutive"? Kanskje bare referere til "Note on Ohm's law" i Basics?}. 

We note that we have used the subfix "t" to indicate that ${\bf i}_\mathrm{t}$ is a macroscopic current density through \textit{tissue}, defined as current unit per tissue cross section area. \tvnnote{Jeg foreslaar aa fjerne resten av dette avsnittet, og bare henvise til Sigma-kapittelet. Hvertfall hvis vi ikke noedvendigvis tror paa at stroem gaar mest ekstracellulart.}Tissue currents are assumed to move predominantly through the extracellular part of tissue \cite**{Robinson1968,Nunez2006}, but may contain contributions from intracellular pathways
...
it will enclose the intracellular spaces of multiple neurites as well as the extracellular space between them. 
...
At the same time, there is both experimental (see e.g., \cite**{Okada1994}) and theoretical (see e.g., \cite**{Meffin2014}) 
...
we can reformulate \fref{eq:VC:pointsources} by using the the multipole expansion \cite**{Nunez2006}\tvnnote{Syntes vi burde faa det bedre frem at dette foreloebig ikke er noen approksimasjon}:
\begin{equation}
...
In contrast, the amplifier must have a very high impedance \tvntxt{(a generalized version of resistance that can incorporate capacitive effects and be frequency dependent)}, 
...
\tvntxt{We start by ignoring any possible temporal signal distortion introduced by the
...
To account for the effect of the electrode size we can use \ghtxt{\sout{the point-electrode approximation}VC-theory}\tvnnote{syntes det er fint aa ha med at dette egentlig bare er en triviell utvidelse av punk-kilde approksimasjonen} to calculate $V_\mathrm{e}$ at many different points across the electrode surface 
...
The disc-electrode approximation accounts for the physical extent of the electrodes, and therefore also for the spatial filtering effects introduced by electrodes that are large compared to the spatial variations of the $V_\mathrm{e}$. See \fref{sec:Spikes:electrode_size} for an example of this approach applied to calculating extracellular 
...
Large multi-contact electrode shanks can be expected to have a substantial effect on close-by extracellular potentials, since they represent large, non-conducting volumes \cite**{Moffitt2005,Lempka2011,Mechler2012,Buccino2019}. 
\tvntxt{While such effects can be expected to be
negligible for small microwires \cite**{Buccino2019},
it has been demonstrated that large probes can amplify or dampen recorded potentials from nearby cells by almost a factor of two, depending on whether the cell was in front of or behind the electrode shank \cite**{Moffitt2005,Buccino2019}.
Efforts to model large non-conducting volumes such as this are also briefly covered in \fref{sec:Sigma:nonhomo}, which concerns modelling of
planar boundaries to non-conducting regions.
Finally, note that insertion of large probes into neural tissue 
can cause tissue damage, leading to eudema and encapsulation \cite**{Moffitt2005}, as well as cell death, but we do not cover such issues here.}
%%%%%%

\subsection{\blue{Signal distortion from recording equipment}}
\label{sec:VC:elec_distrotions}
\tvntxt{It is commonly assumed that if the proper recording equipment is used, 
electrode impedance or other electrode properties should not substantially distort recorded potentials. In other words, it is assumed that the recorded potential, $V_\mathrm{Rec}$ is identical to the average potential across the electrode surface, which we will here refer to as $V_\mathrm{True}$ 
\cite**{Martinsen2008,Moulin2008,Nelson2008,Nelson2010}.
Note that $V_\mathrm{True}$ here includes the effects from the electrode size and the presence of the electrode itself. The assumption that $V_\mathrm{Rec}$ equals $V_\mathrm{True}$
is typically referred to as assuming "ideal" electrodes.
...
Apart from the issue \tvntxt{discussed above with the unobservable DC-potential, } 
%\ghnote{Hvilke issues? Alle? De som angaar DC-potensialet? Eller mener vi "despite the issues?"}\tvnnote{Jeg tenkte paa DC-potensialet. Min forstaaelse av dette er at siden Ze er veldig frekvens-avhengig, blir ikke Za' mye stoerre enn Ze' naar f er liten, men jeg vet ikke om dette er riktig tolkning.}
...
\tvntxt{When we record extracellular potentials in the brain, we are in principle free to arbitrarily choose the location of the reference electrode,
