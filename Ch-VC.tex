...
we may assume that some (unknown) fraction of the tissue current density
(i) crosses membranes and evokes (unknown) current sources additional to those simulated ($C \neq C_\text{all}$), 
and (ii) travels through intracellular pathways (${\bf i}_\text{t} \neq {\bf i}_\text{e}$).
...
As we have seen, the tissue current density is typically assumed to be linear and ohmic, 
so that ${\bf i}_\text{t} = -\sigma_\text{t} \nabla V_\text{e}$. Based on this assumption,
...
To perform such measurements, no assumptions need to be made as to whether the
current pathways between the stimulus and measurement electrode
are purely extracellular or partially intracellular, but there
is strong experimental evidence that its measured magnitude
indicates that a fair fraction of the tissue currents take intracellular pathways \cite**{Okada1994}. 
The effective tissue conductivity is discussed in greater detail in the next chapter.

In conclusion, when using \fref{eq:VC:CSDit} together with 
an ohmic model for the tissue current, and a measured value for $\sigma_\text{t}$, 
we do not need to make any assumption regarding the pathways 
...
potential gradient $\nabla V_\text{e}$. 
\tvnnote{Boer vise tilbake til der vi sier at det er godt dokumentert at dette stemmer}

\ghnote{Her kom det altsaa et nytt delkapittel basert paa det vi snakket om i mail-utvekslingene. Jeg foelte det var noedvendig aa klargjoere hvorfor teoretisk utleda CSD-ligning inneholder denne rent ekstracellulaere stroemtettheten fordi alle kilder er kjente, mens vi i forward-modelling burde noeye oss med en mer generell tissue-stroemtetthet fordi vi kun kjenner vaare egne kilder. Kapittelet ender litt utilfredsstillende, synes jeg, med at vi da vaersaagod faar anta at baade intra- og extracellulaere tissue-stroemmer maa avhenge lineart av det EKSTRACELLULAERE potensialet, noe som saann i farta kan hoeres litt snodig ut. 
Jeg vet ikke hva jeg skal si om dette. Det  er sikkert rimelig nok, men laater ikke som noen selvfoelge. Det er sikkert usant dersom "the unaccounted for sources" inneholder festglade nevroner som lager mye rabalder, men sikkert noksaa sant dersom det kun er snakk om smaa ephaptiske polariseringer, slik jeg indikerte i teksten, men ikke vaaget aa skrive helt ut. Tar gjerne imot kommentarer paa dette.}
