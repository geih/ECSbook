...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Eq.~\ref{eq:VC:pointsources} is referred to as the point-source approximation \cite**{Holt1999,Pettersen2008a}, since it approximates the neuron as a set of point current sources, i.e., the neuron delivers to the extracellular space a singular current source per neuronal segment, located in the segment midpoint. 
...
\tvnnote{We should discuss this. Some info is needed, but I think it should be much less than we would include for papers to avoid a large number of very long figure captions. We will of course find a way of linking to the code used in generating the figure.}
}
...
\ghnote{GH: Det som staar paa Wiki-siden er multipolekspansjon basert paa ladninger. Vaar er basert paa stroemkilder. Premisset for vaar multipolekspansj er da CV-teori, saa jeg synes stoffet hoerer hjemme her. Vi har en mer substansiell utledning i appendix, og vi bruker stort sett metoden kun i tilfeller da vi gjoer dipol-forenklingen. Derfor synes }
...
\tvnnote{Enig, vi boer nok flytte en del av dette dit.}
