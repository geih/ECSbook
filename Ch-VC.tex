...
allow us to compute the transmembrane 
\gen{Skal vi bruke "transmembrane" eller "membrane" currents?} currents at various locations on the neural membrane. 
...
at any given point in space. Volume conductor (VC) theory is the fundament \gex{foundation?} for forward modeling 
...
A fairly general starting point for VC theory is the continuity equation \gen{"continuity equation" for what? charge?} (\fref{eq:Basics:continuity1}) 
...
\label{eq:VC:ohmici}
\end{equation}
\gen{Er det riktig aa bruke subscript "t"?}
...
\gen{Antagelse 4 er vel litt annerledes enn de tre f{\o}rste som vel er fundamentale for VC. Den 4. antagelsen om konstant sigma-t er kun en forenkling vi gjoer her. Nevne?} 
...
while still sufficiently small for a reference volume to be either entirely inside or entirely outside
of a membrane. 
\ghnote{Er dette ok? Membranen er noe dritt. Foelte jeg burde hatt den med i siste setning, 
men den er ikke helt med paa skala-notene vaare. Jeg regnet tidligere ut at $(10 nm)^3$ ECS bare inneholder 2 kaliumioner,
og ikke alltid er stort nok til at vi kan stole paa en deterministisk kontinuumsbeskrivelse. Og der er vi jo  paa membrantykkelsen.... Saa jeg loeste dette her ved ikke aa nevne membranen. }
As such, \fref{eq:VC:Sergey1} would not be practical to use for describing 
large-scale tissue processes: We would have map up the detailed structure of brain tissue with high resolution, 
to account for how the very definition of ${\bf i}_\text{tot}$ varies spatially 
between being an intracellular, extracellular or transmembrane current.
...
\ghnote{Vi har styret litt med dette, men jeg mener aa ha forstaatt Sergey naa. Cellular betyr intracellular + membrane. Dette gir jo mening jmf Gauss divergesteorem: Divergensen av det inni er det som kommer ut. Sergey skriver det ogsaa: averaging over cytoplasm and membrane.}
...
\section{\blue{\ghtxt{Modeling recorded potentials}}}
