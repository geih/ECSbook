...
\subsection{\blue{Impressed versus primary currents}}
\label{sec:VC:C4}\index{Inverse problem}
Readers well versed in the literature on electric and magnetic fields in the brain
may have come across terms such as \textit{the impressed current density}\index{Impressed current}
and \textit{the primary current density}\index{Primary current}.
...
\end{equation}
where it is implicit that the current densities have been subject to some (not yet defined)
spatial averaging, which is why the derivation of the CSD equation in \fref{sec:VC:C2} is more formal.
In both the equations above, the first term on the right has to do with cellular current sources
...
\Fref{eq:VC:inverse1} is defined on a fine-grained scale, where
${\bf i}_\mathrm{i}$ is the impressed current density\index{Impressed current}, 
which exclusively reflects transmembrane neural currents and is nonzero 
only on sites of cellular activity, and where $\sigma = \sigma({\bf r})$
is the local conductivity. As argued by \citeasnoun**{hari1986}, a change in $V_\mathrm{e}$
can generally reflect either a change in ${\bf i}_\mathrm{i}$ or a change (however local) in $\sigma$, 
\gen{Litt usikker paa om jeg skjoenner formuleringen helt. Ve endres jo ogsa med posisjon...}
and both these changes are likely to take place near membranes during the activation of a synapse or ion channel.
As we do not have sufficient insight into the tissue microstructure
to describe a $\sigma$ that varies dynamically and locally on this scale, 
it has been suggested that \fref{eq:VC:inverse1} should be replaced with
\fref{eq:VC:inverse2}, where the involved variables are considered 
on a coarse-grained (tissue averaged) length scale of about 1 mm \cite**{tripp1983,hari1986,sarvas1987,Ilmoniemi2019}.

In \fref{eq:VC:inverse2}, the problem is tackled at the same spatial scale
as our previously derived CSD-equation (\fref{eq:VC:CSDit}), 
so that $\sigma_\text{t}$ has the interpretation of a macroscopic tissue conductivity as we have defined it previously. 
The primary current density, ${\bf i}_\mathrm{p}$, is defined so that it
incorporates both ${\bf i}_\mathrm{i}$ and the above mentioned
microscopic changes in conductivity \cite**{tripp1983}.
As ${\bf i}_\mathrm{p}$ is believed to predominantly reflect
...
However, whereas ${\bf i}_\mathrm{i}$ by definition includes only transmembrane contributions, 
${\bf i}_\mathrm{p}$ is defined in a less binding way 
as ${\bf i}_\mathrm{p} = {\bf i}_\text{tot}-\sigma_\text{t} {\bf \nabla} V_\mathrm{e}$, 
i.e., as "all current that is not explained by the macroscopic tissue current" \cite*{Ilmoniemi2019}.
\gen{Hmm... Fortsatt litt uklart for meg. Kan str{\o}emmer i ECS paa mikroskopisk nivaa ogsaa innga i primary current?}

As we noted above, the distinction between impressed and primary current densities
are mainly made by scientists addressing the inverse problem.
The reference to a primary (instead of an impressed) current density then amounts to
admitting that not everything measured by necessity must reflect transmembrane 
sources, but possibly also secondary local conductivity changes, or even
other sources, such as e.g., extracellular diffusion (see \fref{chap:Eldiff}).
...
(or diffusive currents) in the tissue near simulated cellular current sources 
unless we explicitly put them into the model. 

By requiring current conservation ($\nabla\cdot{\bf i_\text{tot}}=0$), 
and by identifying the current source density as $C = -\nabla\cdot{\bf i}_\mathrm{p}$
(equivalent to \fref{eq:VC:Sergey4}), 
we see that \fref{eq:VC:inverse1} is equivalent with the CSD-equation (\fref{eq:VC:CSDit}).
