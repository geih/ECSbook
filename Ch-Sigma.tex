...
to be less attenuated with distance compared to high-frequency spikes \cite**{Logothetis2007}\tvnnote{Dette er kanskje egentlig mer komplekst. I formalismen vi brukte i Miceli2017 (ved aa bare legge til en kapasitiv (kompleks) del), vil ikke forskjellige frekvenser avta forskjellig med avstand, 
...
enten den er frekevsavhengig eller ei. Ok?}\tvnnote{OK for meg}
...
of $\sigma_\mathrm{t}$ and ${\bf E}$ \cite**{Bedard2009}\footnote{\tvntxt{One can move between the time domain and the frequency domain through Fourier transforms.}}. Only in the case when 
the $\sigma_\mathrm{t}$ is frequency independent, it is valid also in the time domain. 
\ghnote{Skal vi si mer om saken, eller holder dette? Vi har ikke forklart hva time-domain og frequency-domain betyr.} \tvnnote{La paa en fotnote. Kanskje kommentere paa forholdet mellom "I(omega)" og "vec i"?}

We also note that we in the above focused on the frequency dependence due to capacitive properties in the medium.
\tvntxt{\sout{In addition to this, the}This can be further generalized so that the} real part of the conductivity and the permittivity may have "individual" frequency dependencies, 
...
\tvnnote{Jeg vet ikke. Det er jo mer generelt, men samtidig vanskeligere aa se for seg en biofysisk mekanisme som kan gjoere realdelen frekvensavhengig. Kanskje Ih-kanaler for eksempel, som aapner aa lukker seg basert paa frekvensen i feletet?}
...
\tvntxt{This fits nicely with the earlier discussed measured impedance spectrum
