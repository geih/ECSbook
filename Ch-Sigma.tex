...
\ghtxt{The tortuousity accounts for the fact that agents moving extracellularly through the medium 
cannot travel in straight lines, but must take detours around cellular obstacles.
...
fraction of tissue \cite**{Nicholson1981,Sykova2008}. \ghtxt{Several studies have been performed
...
\ghnote{Tankegang rundt notasjon:
Vi har tidligere kalt tissue-conductivity for $\sigma_\text{t}$, og vi har sagt at 
$\sigma_\text{t}$ kan vaere frekvensavhengig. Ergo tenker jeg at vi boer
kalle den $\sigma_\text{t}$ ogsaa naar den er kompleks, selv om andre (Nunes for eksempel)
bytter navn paa den og kaller den $\sigma_\text{c}$ naar den blir kompleks. 
Vi trenger da et navn paa realdelen, og jeg har nedenfor kalt den $\sigma_\text{Rt}$,
the real part of tissue conductivity for aa skille den fra the real part of another conductivity.
Notasjonen $\sigma_\text{R}$ brukes ogsaa i Miceli-artikkelen. Er dere ok med dette?}
...
An electric field oscillating with angular frequency $\omega = 2\pi f$ 
...
to indicate that an entity is complex. \ghnote{Ok med asterix? Burde vi brukt miraculix?}
...
\label{eq:Sigma:itot3}
\end{equation}
We note that \fref{eq:Sigma:itot3} is only valid in the frequency domain
...
\label{eq:Sigma:itot2}
\end{equation}
By comparing this to \fref{eq:Sigma:itot3}, we see that the complex conductivity is defined by:
\begin{equation}
...
and we have mainly though of it as being purely resistive.
However, to be precise, we from now on let $\sigma_\text{Rt}$ denote the resistive 
part of the tissue conductivity, 
i.e., the real part of the (possibly) complex and more general tissue conductivity, $\sigma_\text{t}^*$.
...
\begin{equation}
V_\mathrm{e}^*({\bf r}, \omega) = \frac{I^*(\omega)}{4\pi (\sigma_\text{Rt} + \mathrm{j} \omega \epsilon_\text{t}) r}, 
\label{eq:Sigma:pointsource_freq_dep}
\end{equation}
In \fref{eq:Sigma:pointsource_freq_dep}, $I^*(\omega)$ represents a singular point source oscillating at a single frequency, 
$\omega$. However, the problem is linear, which means that for signals composed of multiple frequency components, 
...
In a more general case, also the real parts of the conductivity and the permittivity 
