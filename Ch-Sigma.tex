...
is the conductivity for the macroscopic current per unit \textit{tissue} cross section area. 
...
The motivation for introducing the subfix "t" here is mainly that we need to distinguish it from some other possible
conductivity definitions, which we introduce below.}
...
so that brain tissue can be treated as a continuous, porous medium \cite**{Gardner-Medwin1980,Nicholson1981,Nicholson2001,Gratiy2017}. 
...
\gen{Artikkelen til Klas og co i PNAS er et eksempel hvor dette likevel er gjort, men da paa et veldig lite volum.}
...
When treated as a continuous porous medium, the structure of brain tissue is 
defined by two key parameters \cite**{Nicholson1981}. 
The first parameter, $\alpha$, is the fraction of the tissue volume that is extracellular space:
\begin{equation}
\alpha = \frac{\mathrm{Vol}_\mathrm{extracellular}}{\mathrm{Vol}_\mathrm{tissue}}
\label{eq:Sigma:alpha_dog}
\end{equation}

The second parameter, $\lambda$, is the tortuousity\index{Tortuosity} of the extracellular medium.
The tortuosity may have different definitions in different disciplines, 
but in studies of neural tissue it has simply been defined as the ratio
between the diffusion constant $D^*$ in the tissue compared to its value $D$ in a free medium \cite**{Sykova2008}:
\begin{equation}
\lambda = \sqrt{\frac{D}{D^*}}, 
\label{eq:Sigma:tort_og_svie}
\end{equation}
As reviewed in \cite**{Sykova2008}, the tortuousity may reflect a a variety of different hindrances
faced by diffusive agents in the medium, including 
(i) physical obstacles to free diffusion represented by cell bodies, which
force diffusive agents to take detours around them,  
(ii) transient trapping of diffusive agents in dead-space microdomains, such as cellular cavities, 
(iii) chemical interaction between diffusive agents and molecules in the extracellular matrix, 
(iv) unknown electric interactions between diffusive agents and molecules in the extracellular matrix, 
and (v) physical obstacles to free diffusion represented by molecules in the extracellular matrix. 

The parameters $\alpha$ and $\lambda$ can be estimated experimentally
by measuring diffusion of tracer molecules believed to stay confined within the extracellular
fraction of tissue \cite**{Nicholson1981,Sykova2008}. Several studies have been performed
to determine these quantities. We do not present them in any detail here,  
but the interested reader can visit the comprehensive review paper by \citeasnoun**{Sykova2008}.
Typical estimates are $\alpha = 0.2$ and $\lambda = 1.6$,
although these values can vary between brain regions, 
and also locally within a brain region, due to brain-state dependent cellular swelling or shrinkage \cite**{Sykova2008,rasmussen2020}.
...
\ghtxt{As a starting point, we assume that all these three conductivities are 
frequency independent (DC conductivities), but comment on this assumption
when introducing $\sigma_\text{t}$}.
...
that travels exclusively through the extracellular part of brain tissue. 
...
It is worth taking a brief side-step to compare \fref{eq:Sigma:sigmat} to corresponding relationships
obtained through more theoretical approaches. An early example is the Maxwell-Rayleigh formula
for the resistivity of medium with a homogeneous suspension of identical non-conductive bodies.
This equation was listed as equation 4.1 in \citeasnoun**{Nunez2006}. 
Expressed in terms of the parameters 
$\sigma_\text{e}$, $\sigma_\text{saline}$ and $\alpha$, this equation takes the form:
\begin{equation}
\frac{\sigma_\text{e}}{\sigma_\text{saline}} = \frac{\alpha}{1+h(1-\alpha)},
\label{eq:Sigma:MaxRay}
\end{equation}
where the new parameter $h$ depend on the structure of the non-conducting bodies, 
so that $h=1/2$ for spheres, and $h = 1$ for cylinders 
(assuming that the current direction is normal to the cylinder axes).  
By comparing \fref{eq:Sigma:MaxRay} and \fref{eq:Sigma:sigmat} 
we find that they are in agreement if:
\begin{equation}
h = \frac{\lambda^2-1}{1-\alpha}
\label{eq:Sigma:Comp_MaxRay}
\end{equation}
For the typical values $\lambda = 1.6$ and $\alpha = 0.2$ this gives us $h \approx 2$, 
meaning that the cell bodies in brain tissue makes an extracellular journey
roughly four and two times as awkward an "extra-bodily" journey through a medium
with suspended spheres and cylinders, respectively. 
All in all, this does not sound too unreasonable. 
However, it should be noted that the theoretical treatments rest rest on some assumptions
that are violated. For example, it is not possible to pack spheres tight enough to obtain
an extracellular volume fraction as low as $\alpha = 0.2$
...
Already in 1981, \citeasnoun**{Maxwell1954}
derived a relationship for the conductivity of a conductive medium when filled with 
...
\ghnote{Du hadde nok et godt poeng her. Jeg droppet den teoretiske definisjonen
av $\lambda$ som ECS-path delt paa euklidisk distanse, og anga den som
effektiv diff-konst. $D^*$ delt paa fri diff-konst. $D$, som er definisjonen
i foelge Sykova 2008. Jeg dropper da aa spekulere i hvorvidt siste 
definisjon kan vaere et estimat paa den foerste. 
Jeg tror dog at vi kan stole paa at $\lambda$ er et maal som
gjelder ren ekstracellulaer transport. 
Eller - vi kan i hvert fall stole paa at Nicholson mener den er det.}

\ghnote{Klas nevnte noe om teoretiske estimater (Tao og Nicholson).
En oversikt over diverse teoretiske studier gis i Sykova 2008.
Sykova mener at disse ikke har saa mye for seg da alle er basert paa
diverse antakelser om strukturen til celler. For aa forklare den "observerte"
lambda maa man derfor inn og postulere slike cellulaere kaviteter
som holder ioner som gissel ei stund foer de slippes laus igjen.
De enkleste slike teoretiske greier ble gjort av Maxwell allerede paa
slutten av 1800-tallet, og var basert paa konduktansestimater i 
medier fyllt med "isolating bodies". 
Jeg kikket paa noe lignende tidligere, da Torbjorn spurte meg hvordan vaar sigma
henger sammen med Nunez (ligning 4.1.), som er den saakalte
Maxwell-Reileigh formelen: $\sigma/\sigma_\text{saline} = (1+h\gamma)/(1-\gamma)$. 
Her er $\gamma$ fraksjonen av rom okkupert av "insulating bodies", mens 
$h$ er strukturavhengig og oppgis til 1/2 for kuler og 1 for sylindre. 
Sammenligner vi dette med vaar (dvs. Okada og Nicholsons)
$\sigma/\sigma_\text{saline} =\alpha/\lambda^2 = (1-\gamma)/\lambda^2$
og bruker $\alpha = 0.2$ og $\lambda = 1.6$, faar jeg samsvar hvis $h=2$, 
noe som ikke laater saa hysterisk ille.
Tissue-strukturen er altsaa et terreng som er fire ganger saa ulendt
som en gjeng med kuler, og dobbelt saa ulendt som en gjeng med sylindere. 
Et annet fun fact her er at det er umulig aa stable kuler tett nok til aa faa en
ECS fraksjon paa 0.2 (0.26 er nedre grense). Det var visst derfor
Tao og de begynte med noen oktraedron og greier. 
Sykova 2008 konkluderte med strukturantakelsene alltid
vil vaere et svakt punkt, og det virker som hun synes det er best 
aa bare maale det paa Nicholson-vis. 
Jeg valgte aa ikke gaa inn paa disse sakene, 
for det blir fort til at vi maa skrive ganske mye om vi gjoer det.}
...
\ghtxt{However, if tissue currents to some degree interact with cellular membranes, they will become
dependent on the cable properties and orientation of cells, which could cause
$\sigma_\text{t}$ to differ from $\sigma_\text{e}$ in two main ways.
Firstly, interactions with capacitive neural membranes could give $\sigma_\text{t}$ 
a frequency-dependence not present in $\sigma_\text{e}$, a topic that we
discuss in further detail in \fref{sec:Sigma:f-independent}. Secondly, 
if tissue currents to some degree cross neural membranes,
it means that the tissue will offer intracellular pathways for
macroscopic currents in addition to the extracellular ones. 
In that case, we would expect the DC component
of $\sigma_\text{t}$ to be greater than $\sigma_\text{e}$.}
...
\ghnote{Ja, du snakker om dette, men jeg vet ikke. Hovedpoenget er vel at det kommer EKSTRA space, ikke shortcuts. 
Vurder om det ikke er tilstrekkelig, det jeg skriver i formuleringen over.}
...
\ghnote{Ok naa? La inn "proportional to" istedenfor alligned with, etter forslag fra Gaute. Klas foreslo
"the sum of them" istedenfor "both of them", og selv om Klas sitt forslag sikkert er mer vanntett,
synes "both of them" korrekt. Det er veldig lett for meg aa godta at den ekstracellulaere stroemmen
boer vaere proporsjonal med det ekstracellulaere feltet. Hvis vi godtar at det er slik, maa ogsaa den intracellulaere
vaere det. }
...
If \tvntxt{the conductivity of neural tissue is frequency dependent}, 
extracellular potentials will be distorted \tvntxt{\sout{as they propagate through}by} the tissue, 
...
at least up to frequencies in the megahertz range \cite**{cooper1946,Martinsen2008}.
...
but will in the current chapter focus on endogenous signals.
...
\label{sec:Sigma:Complex}
\ghnote{Kan Gaute/Klas sjekke at om seksjonen ser akseptabel ut?}
\ghnote{Det meste av dette delkap skrives for generelt medium - bare siste del
handler spesifikt om tissue.}
...
A field oscillation with a single frequency component with angular frequency $\omega = 2\pi f$ 
...
If we insert the field (\fref{eq:Sigma:Eomega}) into \fref{eq:Sigma:itot}, we get
\begin{equation}
\hat{{\bf i}}(t) = \sigma_\text{R}\hat{{\bf E}}(t) +  \mathrm{j}\omega\epsilon\hat{{\bf E}}.
\label{eq:Sigma:itot}
\end{equation}
By comparison with \fref{eq:Sigma:WhatWeWant}, we see that the
...
In \fref{eq:Sigma:WhatWeWant}, we can also eliminate the factor $e^{\mathrm{j}\omega t}$
from both sides of the equations, to get an equation,
\begin{equation}
\hat{{\bf i}}_0(\omega) = \hat{\sigma}(\omega){{\bf E}}_0,
\label{eq:Sigma:f-space}
\end{equation}
that no longer contains the time variable. We say that \fref{eq:Sigma:f-space} 
is only defined the frequency domain. It relates the frequency and phase
of the current density to the frequency and phase of the electric field,
but does not contain information about where we are in the cycle. 
\footnote{One can move between the time domain and the frequency domain through Fourier transforms.}.

If brain tissue has a frequency dependent conductivity, we may denote it by $\hat{\sigma_\text{t}}(\omega)$.
We note that we up until this point have denoted the tissue conductivity simply by $\sigma_\text{t}$, 
as we have mainly spoken of its resistive properties. 
However, to be precise, we will for the remainder of the current subchapter
let $\sigma_\text{Rt}$ denote the resistive part of the tissue conductivity, 
i.e., the real part of the (possibly) complex and more general tissue conductivity, $\hat{\sigma_\text{t}}(\omega)$.
...
can be replaced with a frequency dependent point-source equation \cite**{plonsey1967,Bossetti2008,Miceli2017}:
\begin{equation}
\hat{V_\mathrm{e}}({\bf r}, \omega) = \frac{\hat{I}(\omega)}{4\pi (\sigma_\text{Rt} + \mathrm{j} \omega \epsilon_\text{t}) r},
\label{eq:Sigma:pointsource_freq_dep}
\end{equation}
which is valid in frequency space. 
In \fref{eq:Sigma:pointsource_freq_dep}, 
$\hat{I}(\omega)$ represents a singular point source oscillating at a single frequency, $\omega$. 
However, the problem is linear, which means that for signals composed of multiple frequency components, 
one can solve \fref{eq:Sigma:pointsource_freq_dep} for each component separately, 
and then sum up the individual solutions. 

We note that we in the above focused on the frequency dependence due to capacitive properties in the medium.
In a more general case, the real parts of the conductivity and the permittivity 
can have "individual" frequency dependencies: 
$\sigma_\text{Rt}=\sigma_\text{Rt}(\omega)$ and $\epsilon_\text{t} = \epsilon_\text{t}(\omega)$.
Importantly, these individual frequency dependencies are not independent, 
as a certain relationship between $\sigma_\text{Rt}(\omega)$ and $\epsilon_\text{t}(\omega)$ is required
...
$|\sigma_\text{Rt}(\omega)|$ and the induced phase-shift $e^{-\mathrm{j} \theta(\omega)}$ \cite**{bechhoefer2011,Miceli2017}. 
\ghnote{Bytta $f$ med $\omega$ i siste parentes, siden vi har brukt $\omega$ ellers. Regner med at $\theta$ er en generell funksjon, slik at dette er ok. Sjekk at det er ok og fjern kommentar.}
\ghnote{Foeler det trengs noe mer her. I det minste noen referanser? La til ref til Miceli, der det staar noe om Bode. Andre refs?}
\gen{Ja, mener vi fant fram til noen gamle solide referanser paa dette i forbindelse med arbeid med Miceli, 2017}
\ghnote{La til Bechhoefer, som ble sitert i Miceli. Kan du se paa dette, Torbjorn? Er vi ferdige her?}


\subsection{\blue{Estimates of the capacitive effects in the brain}}
...
\end{equation}
as the charge relaxation time in a linear medium \cite**{Grodzinsky2011}. To see this, we start with taking the divergence ($\nabla\cdot$) of \fref{eq:Sigma:itot}. Since the total current must be conserved ($\nabla\cdot{\bf i} = 0$), this gives us:
\begin{equation}
...
\ghnote{Nei, jeg tenker meg at jeg hittil har vaert generell. $\tau$ og $\sigma_\mathrm{R}$ kan defineres for generelle medium. Som du ser under kan tau vaere membrantidskonstanten om den regnes ut inni membran-mediumet.}
...
that it rightfully has the interpretation of a charge relaxation time: $\tau_\text{m}=\epsilon_\text{m}/\sigma_\text{m}$.
...
\ghnote{Nei, ikke jeg heller, og har derfor vaert usikker paa meg selv. Men Nunes plugga $\tau_m$ inn som om den var charge-reaxation konstanten, saa dette var et forsoek paa aa vise at dette er en rimelig ting aa gjoere.}
...
\ghnote{So, capacitive effects can be comparable with resistive? Her putta jeg inn nyere data fra Wagner. De finner hoeyere $\epsilon_r$ verdier enn tidligerere studier. De studerer in-vivo tissue, mens eldre var ex-vivo. Vi kan ikke paastaa at det siste ratioet er mye mindre enn 1, saa her blir det problemas. Det hjelper ikke aa kjoere 1000 Hz heller. Ratioet der blir faktisk enda hoeyere, 0.7. Gammel tekst foelger nedenfor.}
...
\ghnote{Alt tyder paa at det er vakuum. Jeg er heller ikke helt inneforstaatt med hvordan dette gaar til, men 
Nunez sier $\epsilon_r \sim 10^6$ for muskelvev. Jeg tall i samme stoerrelsesorden andre steder, 
som sier eksplisitt at det er relativt til vakuum.}
\gen{Tror det finnes det andre nyere referanser her i tillegg til Gabriel.}
\tvnnote{Kan se paa Wagner et al. 2014 for eksempel.}
\ghnote{Ja, jeg gjorde jeg det, se over. Da ble det skikkelig troebbel.}
In comparison, the conductivity varies little with frequency, 
...
\ghnote{Denne avsluttende argumentasjonen faller hvis vi bruker Wagner-dataene. Konklusjonen er kanskje rett og slett at kapasitive effekter ikke er saa uviktige?}
...
In principle, one can still always solve the general CSD equation (\fref{eq:VC:ohmici}) 
for arbitrarily complex geometries with varying conductivities using numerical methods, 
like the Finite Element Method (FEM) \cite**{Logg2012}. 
