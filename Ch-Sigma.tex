...
\end{equation}
We have previously shown how this definition of the conductivity follows from the Nernst-Planck equation (see \fref{eq:Basics:sigma_conc}). Here $z_{k}$, $D_k$ and $c_{k}$ denote the valency, diffusion coefficient and concentration, respectively, of ion species $k$, while $F = 96485.3\, \si{\coulomb\per\mol}$ is the Faraday constant, $R = 8.314 \,\si{\joule\per\kelvin\per\mol}$ is the gas constant, and $T$ (\si{\kelvin}) the temperature.

The concentrations of the most abundant ions in the extracellular space of the brain were listed in \fref{tab:Neuron:ion-concentrations}. To compute $\sigma_\text{saline}$ for this solution, we also need the diffusion coefficients of the involved ions, which are given in \fref{tab:Sigma:diffconsts}. If we insert the values from \fref{tab:iNeuron:on-concentrations} and \fref{tab:Sigma:diffconsts} into \fref{eq:Sigma:sigma1}, and assume a body temperature of $T = 310\, \si{\kelvin}$, we get a conductivity $\sigma_\text{saline} = 1.72 \, \si{\siemens\per\metre}$ This agrees well with experimental measures of $\sigma_\text{saline}$, which typically report values in the range 1.3-1.8~\si{\siemens\per\metre}\cite**{Okada1994,Baumann1997,Logothetis2007,Martinsen2008,Miceli2017}.
...
Measured values of vary quite much between different experiments, but tend to be $\geq 0.2 \, \si{\siemens\per\metre}$, and thus higher than the value $\sigma_\text{e} = 0.134\, \si{\siemens\per\metre}$ that we estimated above. The reason why 
the estimated $\sigma_\text{e}$ is lower than $\sigma_\text{t}$, is expectedly that a fraction of the real tissue currents do travel through intracellular pathways. For example, in cerebellum, it has been estimated that about 50 \% of tissue currents travel through intracellular paths \cite**{Okada1994}. A possible additional explanation is that the extracellular solution contains many ions (such as e.g., H$^+$ and HPO4$^{2-}$ ) that we did not include in Table \ref{Neuron:tab:ion-concentrations} and thus not in our calculation of $\sigma_\text{t}$. However, since concentrations of ions others than those in Table \ref{Neuron:tab:ion-concentrations} are quite low, they will probably have only a minor impact on the conductivity.
