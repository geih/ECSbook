...
in \fref{chap:VC} we introduced volume conductor (VC) theory, a framework that links such transmembrane currents to extracellular potentials, based on some simplifying assumptions regarding how
currents make their way through the surrounding tissue.

In most applications of VC theory, the conductivity of brain tissue ($\sigma_\text{t}$) is assumed to be constant, 
as we also assumed throughout \Fref{chap:VC}. 
Its value is then normally taken from some experimental measurement. 
A commonly used value is $\sigma_\text{t} = 0.3$ \si{S/m}, 
but we discuss other, both experimental and theoretical, estimates later on in this chapter 
...
\tvntxt{infinite, }homogeneous, isotropic and frequency independent.
...
or whether passage is blocked by a nearby membrane (low conductivity) (\fref{fig:Sigma:ECS}{\bf A}). 
...
If it does, \ghtxt{\sout{recorded}} signals will be distorted as they propagate through the tissue, 
which in turn will bias \ghtxt{\sout{recordings}them} towards certain parts of the frequency spectrum. 
...
\ghtxt{\sout{As we stated earlier, 
...
\ghtxt{\sout{If such effects were present, we would for example expect
\tvntxt{measurements of extracellular potentials to be biased towards} the low-frequency LFP signals 
compared to high-frequency spikes LOGOTHESIS2007}
...
\ghnote{Sikkert lurt aa forenkle, men skjoenner ikke helt det du (og Gaute) sier. Uansett saa stusset jeg paa: "Expect measurements of ...."? Det er vel det faktiske ECS-potensialet som blir biased (i dette tenkte tilfellet), 
ikke maalingen av det? Se om du liker den nye versjonen. Jeg har ogsaa et par andre steder stroeket ut ting ala "measurements of" der jeg tror at det vi paastaar ikke handler om maalingen, men om det faktiske signalet.}
...
but will in the current chapter focus on physiological signals.
