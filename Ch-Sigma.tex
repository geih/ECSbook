...
A key parameter or, sometimes, variable in volume conductor (VC) theory is the extracellular conductivity, $\sigma_t$. In most applications of VC theory, $\sigma_t$ is assumed to be a constant, and its value is normally taken from some experimental measurement. Estimates can vary greatly between different recordings, but common values are between 0.2 and 0.5 S/m.% (Fig. \ref{Sigma:fig:freq_dep}). 
...
\end{center}
\caption[]{\textbf{Tortuous medium}  Illustration of cross-section of a piece of neural tissue. Extracellular space (light blue) occupies only about a fraction $\alpha = 0.2$ of the total tissue volume, and has a highly tortuous structure. (A) Tissue is highly inhomogeneous on a microscopic level. (B) on a coarse-grained scales the local inhomogeneities average out, and the tissue can be treated as a homogeneous medium. 
See also \citeasnoun**{Sykova2008}.
}
\label{Sigma:fig:ECS}
\end{figure}

At the microscopic scale, the conductivity \index{Conductivity} $\sigma$ in the brain tissue is highly non-homogeneous and anisotropic. For example, the conductivity in a certain spatial direction $x$ will depend on whether there locally is a free extracellular passage (high conductivity in the $x$-direction), or whether this passage is blocked by a nearby membrane (low conductivity in the $x$-direction). Likewise, also the electric potential $\phi$ will vary greatly over tiny distances, depending on the proximity to membrane Debye-layers \cite**{Pods2013}. 

When we study extracellular potentials, we are typically not interested in these microscopic variations in $\sigma$ and $\phi$, but rather in the values of these entities when averaged over some spatial volume. In practice, the electrodes used in experimental recordings perform such an averaging. Sizes of electrodes used to record extracellular potentials typically range from 5 $\mu$m to 125 $\mu$m in diameter \cite**{Viswam2019}, which is larger than the typical diameter of a dendrite ($\sim$ 1 $\mu$m). Experimental recordings thus give us the average potential over a region in space that typically spans over both extracellular space and several neural and glial processes \tvnnote{Høres litt ut som om det er både intracellulært og extracellulært, eller eventuelt at store deler av elektroden er "dekket" av cellemembran? Hvis vi skal snakke om dette bør vi kanskje nesten nevne at de dyttes bort, men at det blir eudema og encapsulation og alt det der?}. It is thus the dynamics at this coarse grained spatial scale that we will focus on.
