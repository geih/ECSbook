...
In most applications of volume conductor (VC) theory, the conductivity of brain tissue, $\sigma_\text{t}$, is assumed to be a constant, and its value is normally taken from some experimental measurement. Estimates of $\sigma_\text{t}$ can vary greatly between different recordings, but common values are between 0.1 and 0.5~S/m. 

Throughout the previous section, we assumed that $\sigma_\text{t}$ was homogeneous, isotropic and frequency independent. In this chapter we discuss these assumptions. In addition, we discuss some experimental and theoretical estimates of  $\sigma_\text{t}$. Before doing so, however, we introduce the continuous, porous medium approximation in some detail, to show what $\sigma_\text{t}$ actually represents.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
When we presented the theory for modeling single neurons (in \Fref{chap:Neuron}), we may have given the impression that they are solitary creatures living in a vast extracellular space with long distances to their nearest cell neighbors. This mental image may stem from early staining procedures developed by Santiago Ram\'{o}n y Cajal, based on the Golgi-method \cite**{cajal1899}, where only a small fraction of the neurons were stained and observed. Judging from the beautiful images produced by Cajal, the brain appeared rather sparsely populated by neurons, and if this had been the case, the tissue conductivity would have been close to that of the extracellular fluid. 

However, in reality, the brain is so densely packed with neurons and glial cells that if they all had been stained, Cajal's images would have been pure black. As illustrated in \fref{fig:Sigma:ECS}, the extracellular space (colored light blue in the figure) occupies only about 20\% of the the total tissue volume. Currents moving through tissue are typically assumed to move mostly through the extracellular space, while the cells are effectively non-conducting bodies \cite**{Robinson1968,Nunez2006}. As tissue currents are restricted to follow extracellular pathways, and thus only can pass through a fraction of the total tissue volume, we therefore expect the tissue conductivity to be lower than that of the saline solution.

\ghnote{Torbjorn foreslo: "From this, we can already make a first rough estimate of the extracellular conductivity: The extracellular saline solution has a conductivity of about 1.5~\si{\siemens / \metre}, but only occupies about 20\% of the space, meaning that we might expect the extracellular conductivity to be on the order of 1.5~\si{\siemens / \metre} $\cdot$ 0.2 = 0.3~\si{\siemens / \metre}. This value fits well with the values typically measured in experiments, although the calculation itself vastly underestimates the complex electrical properties of neural tissue. " Dette var ganske fint, men tar ikke hensyn til tortuositeten, noe jeg gjoer i punktlisten ikke langt nedenfor her. Frykter derfor at dette grovestimatet vil forvirre leseren, naar et anelsen mindre grovt estimat foelger saa tett nedenfor.}

The extracellular space has a highly tortuous \index{Tortuosity} geometry, with an average \tvntxt{extracellular distance between cell membranes of only} about 40-60~nm \cite**{Sykova2008}. At the micrometer scale, the extracellular conductivity \index{Conductivity} $\sigma$ is \tvntxt{therefore} highly non-homogeneous and anisotropic. For example, the local conductivity in a certain spatial direction will depend on whether there locally is a free extracellular passage in that direction (high conductivity), or whether this passage is blocked by a nearby membrane (low conductivity) (\fref{fig:Sigma:ECS}{\bf A}). 
...
$\sigma_\text{saline}$ (S/m) is the conductivity of the saline solution that fills up the extracellular space. Since this saline solution only exists in the small gaps of space between neurons, $\sigma_\text{saline}$ defines the (very) local conductivity within these gaps, and is not the conductivity of brain tissue as a whole. 

In general, the conductivity depends on the availability of free charge carriers, and $\sigma_\text{saline}$ is therefore determined by the ion concentrations in the saline solution:
\begin{equation}
\sigma_\text{saline} = \frac{F^2}{RT}\sum_{k} D_k z_{k}^2 c_{k}.
\label{eq:Sigma:sigma1}
\end{equation}
We have previously shown how this definition of the conductivity follows from the Nernst-Planck equation (see \fref{eq:Basics:sigma_conc}). Here $z_{k}$, $D_k$ and $c_{k}$ denote the valency, diffusion coefficient and concentration, respectively, of ion species $k$, while $F = 96485.3\, \si{\coluomb\per\mol}$ is the Faraday constant, $R = 8.314 \,\si{\joule\per\kelvin\per\mol}$ is the gas constant, and $T$ (\si{\kelvin}) the temperature.

The concentrations of the most abundant ions in the extracellular space of the brain were listed in \fref{tab:Neuron:ion-concentrations}. To compute $\sigma_\text{saline}$ for this solution, we also need the diffusion coefficients of the involved ions, which are given in \fref{tab:Sigma:diffconsts}. If we insert the values from \fref{tab:iNeuron:on-concentrations} and \fref{tab:Sigma:diffconsts} into \fref{eq:Sigma:sigma1}, and assume a body temperature of $T = 310\, \si{\kelvin}$, we get a conductivity $\sigma_\text{saline} = 1.72 \, \si{\siemens\per\petre}$ This agrees well with experimental measures of $\sigma_\text{saline}$, which typically report values in the range 1.3-1.8~\si{\siemens\per\petre}\cite**{Okada1994,Baumann1997,Logothetis2007,Martinsen2008,Miceli2017}.
...
\label{sec:Sigma:Se}
$\sigma_\text{e}$ (\si{\siemens\per\metre}) is the conductivity of the \textit{extracellular medium}\index{Extracellular medium}. It is a theoretical quantity, defined as the conductivity that would be experienced by a \tetxit{theoretical} macroscopic (coarse-grained) current assumed to travel exclusively trough the extracellular part of brain tissue. This current does not move through a 3D volume filled entirely by extracellular saline, but is (i) restricted to move only through the fraction $\alpha$ of the total medium volume that is extracellular, and (ii) must take detours around neural and glial obstacles, as reflected through the tortuousity $\lambda$ \cite**{Nicholson1998,Nunez2006}. 
...
\end{equation}
Typical values for $\alpha$ and $\lambda$ for the extracellular space of brain tissue are 0.2 and 1.6, respectively \cite**{Nicholson1981,Nicholson1998}. With these values, $\sigma_\text{e}$ becomes almost a factor 13 lower than $\sigma_\text{saline}$, and with the above estimate, $\sigma_\text{saline} = 1.72 \, \si{\siemens\per\metre}$, we get a an estimated tissue conductivity $\sigma_\text{e} = 0.134\, \si{\siemens\per\metre}$. 


\subsection{\blue{Conductivity of brain tissue: $\sigma_\text{t}$}}
\label{sec:Sigma:St}
$\sigma_\text{t}$ (\si{\siemens\per\metre}) is the macroscopic (coarse-grained) conductivity of the brain tissue, and is the conductivity experienced by a macroscopic (coarse-grained) current traveling through the tissue. It is $\sigma_\text{t}$ that one normally measures experimentally, and it is $\sigma_\text{t}$ that is the relevant for use in VC theory. 

If currents through brain tissue were indeed confined to stay exclusively in the extracellular part of it, $\sigma_\text{t}$ should be identical to $\sigma_\text{e}$, defined above. However, if currents through brain tissue crosses membranes and to some degree goes through intracellular pathways in addition to the extracellular ones, we would expect $\sigma_\text{t}$ to be greater than $\sigma_\text{e}$ \cite**{Okada1994}.

Measured values of vary quite much between different experiments, but tend to be $> 0.2 \, \si{\siemens\per\metre}$, and thus higher than the value $\sigma_\text{e} = 0.134\, \si{\siemens\per\metre}$ that we estimated above. The main explanation to why the estimated $\sigma_\text{e}$ is lower than $\sigma_\text{t}$, is probably that a fraction of the real tissue currents may travel through intracellular pathways. For example, in cerebellum, it has been estimated that about 50 \% of tissue currents travel through intracellular paths \cite**{Okada1994}. A possible additional explanation is that the extracellular solution contains many ions (such as e.g., H$^+$ and HPO4$^{2-}$ ) that we did not include in Table \ref{Neuron:tab:ion-concentrations} and thus not in our calculation of $\sigma_\text{t}$. However, since concentrations of ions others than those in Table \ref{Neuron:tab:ion-concentrations} are quite low, they will probably have only a minor impact on the conductivity.
...
Consequently, experimental data is consistent with neural tissue being resistive, or with having at most a weak frequency dependency of the order of up to $\sim$30\% over the frequency range of interest for neurophysiology (below a few thousand hertz). This might not sound very small, but modeling studies of frequency dependencies of this order has indicated that it would have a relatively minor effect on measured potentials \ghtxt{of physiological origin} \cite**{Bossetti2008,Tracey2011,Miceli2017}\tvnnote{Add figure adapted from Miceli2017?}. We note however that this is not necessarily true for potentials evoked by extracellular current stimulation using very brief current pulses, because of the very high frequency content of the pulses \cite**{Bossetti2008}. \ghtxt{We will revisit the topic of electrode stimulations in \fref{chap:Stim}, but will here focus on physiological signals.}


\subsection{\red{Complex conductivity}}
\label{sec:Sigma:Complex}
We recall that the the foundation for volume conductor theory is the CSD equation (\fref{eq:Basics:continuity2}):
\begin{equation}
...
\end{equation}
where we have introduced the complex conductivity\cite**{Nunez2006}:
\begin{equation}
\sigma_\text{c} = \sigma_\text{t}(\omega)\left(1 + \frac{j \omega \epsilon(\omega)}{\sigma_\text{t}(\omega)} \right).
\label{eq:Sigma:complex}
\end{equation}
The real and imaginary parts of $\sigma_\text{c}$ correspond to the conductive and capacitive properties of the medium, respectively. As indicated, both these parts can have an individual frequency dependence (through $\sigma_\text{t}(\omega)$ and $\epsilon(\omega)$). However, we note that these "individual" frequency dependencies are not independent: To obey causality, that is, that the extracellular potential originating from a current can not precede the current, there must be a certain relationship between $\sigma_\text{R}(\omega)$ and $\epsilon(\omega)$, which can be found from the Kramers-Kronig relation, or equivalently from the Bode relation between the magnitude of the conductivity $|\sigma(\omega)|$ and the induced phase-shift $\exp ^{-{\bf j} \theta(f)}$. \ghnote{Det stemmer sikkert, men foeler det trengs noe mer her. I det minste noen referanser?}
\ghnote{Burde nok si noe om hvorfor $\epsilon$ og $\sigma_t$ "plutselig" er frekvensavhengige "internt", men vet ikke hva jeg skal si om den saken.} 
...
\begin{equation}
\phi({\bf r}, \omega) = \frac{I(\omega)}{4\pi (\sigma_\text{R}(\omega) + {\rm j} \omega \epsilon(\omega)) r},
\label{eq:Sigma:pointsource_freq_dep}
\end{equation}
\tvnnote{notation must be fixed}

\ghnote{Klas skrev dette: Note also that \fref{eq:Sigma:itot3} is generally only valid in the frequency domain, while in the time domain, ${\bf i}$ must be given as a temporal convolution of $\sigma$ and ${\bf E}$ \cite**{Bedard2009}. However, if $\sigma$ is frequency independent, \fref{eq:Sigma:linearcurrent} will also be valid in the time domain.}
...
\end{equation}
where the last equality follows from the definition of the charge relaxation time (\fref{eq:Basics:chargerelaxationtime}).  \Fref{eq:Basics:relax} also gives us a clear interpretation of the charge relaxation time. It shows that there is no steady-state free charge in the system considered, and that if any nonzero free charge occurs, for example due to sudden change in the electric field, it will decay towards zero with the time constant $\tau$. If we insert for $\tau$, the criterion in \fref{eq:Basics:ratiocondition} simplifies to:
\begin{equation}
2 \pi f \tau \ll 1.
\label{eq:Basics:ratiocondition2}
\end{equation}
Hence, only for frequencies that are very high (approaching the order of one oscillation per charge relaxation time), capacitive effects will be important. For fields of physiological origin, the temporal frequencies are typically quite low, and less than a few thousand \si{\hertz}.Capacitive effects will therefore mainly be important if the charge relaxation time is not much smaller than a millisecond. 

Let us explore whether the criterion in \fref{eq:Basics:ratiocondition} or \fref{eq:Basics:ratiocondition2} is met in three different mediums:

\begin{itemize}
\item \textit{Extracellular saline solution:} To approximate the charge relaxation time for the saline solution, we use (i) that $\sigma_\text{saline} \sim 1.5 \, \si{\siemens\per\metre}$ (see \fref{sec:Sigma:Ssaline}), and (ii) that the relative permittivity ($\epsilon_\text{r} = \epsilon/\epsilon_0$) of dilute salt water solutions is approximately the same as that for water, which for the low field frequencies of physiological signals is $\epsilon_\text{r} \sim 80$ \cite**{hasted1948}. This gives a value $\tau = \epsilon_\text{r} \epsilon_0/\sigma \sim 80\times 8.85\times10^{-12}/1.5 \simeq 0.5 \, \si{\nano\second}$. A charge relaxation time \gex{on} the order of 1 \si{\nano\second} has also been estimated in other works \cite**{Grodzinsky2011,Gratiy2017}), and implies that the displacement current will mainly be important under conditions when the electrical field varies with frequencies in the gigahertz range. The relevant fields with physiological origin vary with frequencies that are orders of magnitude lower than this ($f \sim 100 \, \si{\hertz} $), which implies that capacitive effects can safely be neglected. With $\tau = 0.35 \, \si{\nano\second}$ and $f = 1000 \, \si{\hertz}$, $2 \pi f \tau =  2.2 \times 10^{-6}$, and the criterion in \fref{eq:Basics:ratiocondition2} is clearly met.

\item \textit{Neural membrane:} The permittivity of a parallel plate capacitor is related to specific capacitance through $\epsilon = c_m d$ where $d$ is the distance between the conductors. For the neural membrane ($c_m \sim 10^{-2} \si{\farad\per\square\metre}$ ), the conductors are the intra- and extracellular saline solutions and the distance between them is the membrane thickness $d \sim 10^{-8} \, \si{\metres}$, which gives $\epsilon \sim 10^{-10} \, \si{\farad\per\metre}$. The membrane conductivity of neurons vary temporally depending on what ion channels that are present and activated. For example, the HH model has a specific leakage conductance $\bar{g}_{\mathrm{L}} &=& 0.3 \,\si{\milli\siemens\per\square\centi\metre}$, but a Na$^+$ channel conductance of $\bar{g}_{\mathrm{Na}} = 120 \,\si{\milli\siemens\per\square\centi\metre}$ (\fref{sec:Neuron:active}). 


Neural membranes typically have a time constant $\tau \sim 1 \, \si{\milli\second}$ associated with their capacitive charging.
...
\gen{Kanskje vi kan se paa tilfellet med noen kilohertz ogsaa? Da skulle ogsaa frekvensinnhold i spikes vaere godt dekket.} which in an electromagnetic context can be considered a low frequency. 
...
\subsection{\red{Quasi-magnetostatic approximation of Maxwell's equations}}
...
\tvnnote{Jeg er forvirret igjen: Elektrisk felt i vakuum gir jo ogsaa magnetisk felt, helt uten stroem?}
\begin{equation}
...
Making the quasi-magnetostatic approximation means assuming that the displacement current, or equivalently, capacitive effects\tvnnote{Naa har jeg glemt dette igjen: Hva med om man har et tidsvarierende elektrisk felt i et Ohmsk medium?}, are absent \gex{when computing the magnetic field (?)}, so that \fref{eq:Basics:Max4} simplifies to ${\bf \nabla} \times {\bf B}  =  \mu{\bf i}_\text{free}$. This will be a good approximation provided that the magnitude ${\bf i}_\text{free}$ is much smaller \tvnnote{?} than that of the displacement current. In the following, we will investigate the conditions for this to be a valid approximation. To simplify the analysis, we assume that the current of free charges is purely Ohmic, so that \fref{eq:Basics:totcurrent} becomes:
\begin{equation}
