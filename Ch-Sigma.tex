...
In this chapter we discuss these assumptions and ways to relax them, since neither of them are strictly true.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The first detailed illustrations of neural tissue were the beautiful drawings by Santiago Ram\'{o}n y Cajal
from the late 19th century. 
In these drawings, neurons appeared to be rather solitary creatures, 
living in a vast extracellular space with long distances to their nearest cell neighbors.
Had brain tissue been this sparsely populated, 
its conductivity would have been close to that of the extracellular fluid. 
However, Cajal's drawings were based on data obtained using the Golgi-method \cite**{cajal1899}, 
where only a small fraction of the neurons were stained and observed. 
In reality, the brain is so densely packed with neurons and glial cells 
...
On a small spatial scale, the local conductivity in a certain spatial direction 
...
is therefore highly non-homogeneous and anisotropic. 
...
it is however reasonable to assume that the micrometer scale inhomogeneities average out (\fref{fig:Sigma:ECS}{\bf B}), 
and that brain tissue can be treated as a continuous, porous medium \cite**{Nicholson1981,Gratiy2017}. 
This assumption is further motivated by the lack of any alternative way to approach the problem, 
...
The second parameter is $\lambda$, the tortuousity of the extracellular medium, 
...
although these values vary between brain regions, 
...
If we insert the values from \fref{tab:iNeuron:on-concentrations} and \fref{tab:Sigma:diffconsts} into \fref{eq:Sigma:sigma1}, 
and assume a body temperature of $T = 310\, \si{\kelvin}$, 
we get a conductivity $\sigma_\text{saline} = 1.72 \, \si{S/m}$. 
This agrees well with experimental measures of $\sigma_\text{saline}$, 
which typically report values in the range between 1.3 and 1.8~\si{S/m}\cite**{Okada1994,Baumann1997,Logothetis2007,Martinsen2008,Miceli2017}.
...
\label{sec:Sigma:Se}
$\sigma_\text{e}$ (\si{\siemens\per\metre}) is the conductivity of the \textit{extracellular medium}\index{Extracellular medium}. 
As we define it here, it is a theoretical quantity, 
representing the conductivity experienced by a \textit{theoretical} macroscopic (coarse-grained) current 
assumed to travel exclusively trough the extracellular part of brain tissue. 
...
are 0.2 and 1.6, respectively \cite**{Nicholson1981,Nicholson1998}. 
With these values, $\sigma_\text{e}$ becomes almost a factor 13 lower than $\sigma_\text{saline}$, 
and with the above estimate, $\sigma_\text{saline} = 1.72 \, \si{\siemens\per\metre}$, 
we get a an estimated value $\sigma_\text{e} = 0.134\, \si{\siemens\per\metre}$. 

We note that the extracellular solution contains many ion species (such as e.g., H$^+$ and HPO4$^{2-}$ ) 
that we did not include in Table \ref{Neuron:tab:ion-concentrations} and thus not in our calculation of $\sigma_\text{e}$. 
However, since concentrations of ions others than those in Table \ref{Neuron:tab:ion-concentrations} are quite low, 
they are expected to have only a minor impact on the conductivity.
...
in addition to the extracellular ones, 
we would expect $\sigma_\text{t}$ to be greater than $\sigma_\text{e}$.
...
\section{\red{Frequency dependence of the tissue conductivity}}
...
As we stated earlier, extracellular currents often are assumed to move mostly through the extracellular saline solution
...
since saline is known to be a purely resistive medium, except in
the very high frequency range  ($> 1 \, \si{\mega\hertz}$) not relevant for physiological signals \cite**{Martinsen2008}

A frequency dependent $\sigma_\text{t}$ would, however, not be surprising.
...
a substantial frequency dependence, in particular below 100~\si{\hertz} (\fref{fig:Sigma:freq_dep}). 
A series of more recent studies again seemed to reconfirm the early finings, 
and measured a $\sigma_\text{t}$ that depended very on frequency $\sigma_\text{t}$ \cite**{Logothetis2007,Elbohouty2013,Wagner2014,Dowrick2015,Miceli2017,Ranta2017,Avery2017}. 
Notably, many of these studies consistently measured a moderate increase of $\sigma_\text{t}$ 
by about 20-50\% when the stimulus frequency was increased from a few hertz to some hundreds or thousands of hertz, 
but two of them also measured a similar frequency dependence in (pure) saline, 
indicating that this frequency dependence at least partially originated in the recording equipment \cite**{Logothetis2007,Miceli2017}. \ghnote{Argumentet forutsetter at saline IKKE har en f-avhengighet. Er dette aapenbart, eller noe vi boer kommentere? De som maaler epsilon og sigma for saltloesninger snakker vanligvis om "the low-frequency conductivity", som antyder en eller annen f-avhengighet. Men low betyr nok da alt under gigahertz (charge relaxation), saa det er sikkert rett aa forvente null f-avhengighet for vaare omstendigheter.}
