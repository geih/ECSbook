...
Neural activity is inherently characterized by electric currents that go through neural membranes. These transmembrane currents give rise to electric potentials in the extracellular space of the brain. The main goal of this book is to explore the relationship between the extracellular potential and the underlying neural signaling. 
...
When modeling neurons, it is common to represent their morphologies as branching cables of varying diameter (\fref{fig:Basics:Twostep}B), and to make the approximation that intracellular currents run only in the direction defined by the cables (cf. \fref{eq:Basics:Ohm_r}), i.e., longitudinally along the cables without any radial components. \ghtxt{The extracellular currents remain 3D volume currents (\fref{fig:Basics:Twostep}C)}. The relationship between the extracellular volume currents and the neural transmembrane currents is defined further in \fref{sec:Basics:C}.
