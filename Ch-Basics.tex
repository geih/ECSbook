...
\end{equation}
where we have used the index $t$ to denote that $\sigma_\text{t}$ is the \textit{tissue} conductivity. Generally, $\sigma_\text{t}$ can be a tensor, i.e., it can vary with space and can be different in different spatial directions. If $\sigma_\text{t}$ is the same in all directions, and does not vary across space, \fref{eq:Basics:continuity2} simplifies to the often used form:
\begin{equation}
\sigma_\text{t} \nabla^2{V} = -C.
